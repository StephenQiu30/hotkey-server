...
031 S03 状态校准（2026-09-24）：009 已实现 `JobExecutionFailure` 类型化分类、有限重试、终态失败与同事务 inbox；047 已实现 Worker 子进程监督与未知崩溃重放。禁止宽泛捕获异常后确认消息；分类终态记录不另建 Kafka DLT。用户确认逻辑 Job 从计划到期/即时提交计至 `completed_at`、包含队列和重试，并接受保守故障归属：只有同一 Job 的持久来源失败证据才列来源结果，其余列“未归属”并保守计为失败。`webpage.collect` SLA 已冻结为 30 分钟；只读汇总按 SLA 截止归入半开观察窗，完整成功和按期持久来源失败进入分子，恰好 30 分钟算按期，空分母比率未定义。隔离 backend CI [#35967985243](https://github.com/StephenQiu30/hotkey-server/actions/runs/35967985243) 的 PostgreSQL/Kafka 测试通过（546 passed/12 skipped），Ruff/format/mypy 通过；[contract](https://github.com/StephenQiu30/hotkey-server/actions/runs/35967985156) 与 [runtime](https://github.com/StephenQiu30/hotkey-server/actions/runs/35967985157) workflows 成功。随后增加的只读 CLI 快照入口和参数/JSON 单测已本地验证，当前隔离 CI 待回归。本地数据库未触碰；真实 72 小时观察尚未开始，T01/G3 未全闭环，G4 与产品 AC 保持未通过。
