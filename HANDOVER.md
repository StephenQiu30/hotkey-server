...
1. 按 B03 顺序进入 004 S00，先评审平台与连接 PRD、现有 sources/evidence 边界并冻结 S01 设计；003 的真实上游扩词、调度/主题任务留待 S04 联验，009 S04 等待真实处理器和来源样本，031 S03 留在 M5 长时可靠性阶段继续。
...
本轮已交付 009 S00—S03 与 003 S00—S03：前者覆盖持久任务受理/读取/取消、分类失败、有限重试与任务页，后者覆盖本地主题规则、不可变版本、创建/编辑、列表、独立复制、启停/归档和零写入预览。验证复用当前 PostgreSQL/Redis/Kafka/MinIO 与 env，未启动第二套依赖；003 S03 结束时后端 149 passed/4 skipped、前端 16 tests、OpenAPI 生成/漂移、生产构建、桌面/390×844 真实浏览器及 WCAG A/AA 通过。尚未执行真实业务处理器/来源、上游扩词、SDK 内部计量、真实评分/模型记录、独立对象备份/真实恢复、最后成功/缺口/陈旧传播、003 S04、009 S04、完整 B0 或产品 Acceptance。
