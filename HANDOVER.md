...
- `backend/`：Python 3.12、FastAPI、SQLAlchemy 2、PostgreSQL、Redis、Kafka；底座可运行并已由 uv 锁定依赖。
...
- `src/core/` 管理 `HOTKEY_` 配置、结构化日志、公共错误和 Pydantic 基类；`src/db/` 管理唯一 DeclarativeBase、Engine、Session 和 Alembic 元数据。
...
- 本机默认 PostgreSQL 库包含旧系统历史表，而当前 Python ORM 尚无业务模型。未确定迁移策略前，不得生成或执行删除这些历史表的迁移。
...
2. 明确旧 PostgreSQL 数据的保留、迁移或重建策略，再实现首个业务领域模型与 Alembic revision。
