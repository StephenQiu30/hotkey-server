...
**[007 计划](docs/plans/007-作品资料与上下文计划.md) S00/S01 已完成，Plan 保持 in_progress。** `content` 领域已实现 owner＋来源＋对象类型＋可空作用域＋原生 ID 唯一作品、content/job 发现关系、不可变指标观察、零与未知、等价重投/异义冲突、生命周期读取屏障和 persisted read 同事务证据；只读 `GET /api/contents`、详情、运行时 OpenAPI 生成客户端及 `/content` 桌面/移动页面已闭环。真实 PostgreSQL 串行和双会话并发均保持 1 个作品、2 条发现、2 次观察，删除后立即不可读。后端 180 tests、前端 19 tests、全部静态/构建门禁通过；桌面与 390×844 的正常、部分、错误、空和未登录状态无横向溢出，axe 五次均 0 violation/0 incomplete。复用现有 env 及已运行 PostgreSQL/Redis/Kafka/MinIO，未启动第二套依赖，未新增依赖、脚本或 `.sh`；QA 后开发/测试库均 27 表 0 行。没有稳定原生 ID 的链接仍只作线索；S02—S04、真实来源、正文版本、可见性及产品 0/6 AC 仍待执行，未建立 Acceptance。
...
1. 按 B03 顺序进入 007 S02 Red；Design v1.1 已冻结正文完整度、来源原文/机器提取、quote/repost 目标和发布时间精度，下一步以摘要/截断/仅媒体/机器提取、已解析/未解析关系及重复观察样本证明能力缺失；004 S03 属于 B04，等 B03 内容与来源前置就绪后再进入。003 的真实上游扩词、调度/主题任务留待 S04 联验，009 S04 等待真实处理器和来源样本。
...
本轮已交付 009 S00—S03、003 S00—S03、004 S00—S02 与 007 S00/S01：前者覆盖持久任务受理/读取/取消、分类失败、有限重试与任务页；003 覆盖本地主题规则、不可变版本、创建/编辑、列表、独立复制、启停/归档和零写入预览；004 覆盖连接领域、事实表、状态投影、只读 API/页面与分立证据登记；007 建立唯一作品、多入口发现、不可变指标观察、生命周期/能力证据及列表/详情。验证复用当前 PostgreSQL/Redis/Kafka/MinIO 与 env，未启动第二套依赖；007 S01 结束时后端 180 tests、前端 19 tests、OpenAPI 生成、生产构建、桌面/390×844 真实浏览器及 axe 通过。尚未执行真实业务处理器/来源、真实连接探测/采集、上游扩词、SDK 内部计量、正文版本/可见性、真实评分/模型记录、独立对象备份/真实恢复、最后成功/缺口/陈旧传播、003 S04、004 S03/S04、007 S02—S04、009 S04、完整 B0 或产品 Acceptance。
