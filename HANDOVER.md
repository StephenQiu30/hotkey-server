...
**[039 计划](docs/plans/039-可观测与可运维计划.md) S00/S01 与 S02a 技术子片已完成；S02b 窗口投影代码和本机非数据库验证已完成，Plan 保持 in_progress。** S02a 增加按 owner 隔离的连续三次任务失败摘要 API；S02b 在现有任务详情呈现 owner/current-job 限定的持久窗口范围、状态、停止原因和页数，不推断未记录范围。后端 359 passed/189 skipped、15 架构测试、Ruff/format/mypy，前端 47 tests/lint/typecheck/format/build 通过；运行时 OpenAPI 由应用离线生成后更新客户端，本机合成响应浏览器完成刷新交互、390×844 无横向溢出和 axe 0 violations。隔离 PostgreSQL 投影测试因缺少专用 `HOTKEY_TEST_DATABASE_URL` 跳过，尚未关闭 S02b G3/G4；未写入 `hotkey-server`。完整新鲜度/覆盖缺口、S03—S04 和 039 产品 AC 仍为 0/6，未建立 Acceptance。
