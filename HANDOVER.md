...
- `app/jobs/` 已实现内部持久受理：任务与 `job.accepted.v1` outbox 同事务写入，owner/kind/operation ID 唯一，等价重试返回原任务，异范围重用拒绝。尚无 HTTP、发布器、消费者、租约、检查点或重试状态机。
...
**[031 计划](docs/plans/031-可靠执行与幂等计划.md) S00/S01 已完成，Plan 保持 in_progress。** `jobs` 与 outbox 的原子提交、operation ID 防重、冲突、事务故障和双会话并发已通过本机既有 PostgreSQL 18.4；后端 44 tests 与静态门禁通过。Redis/Kafka/MinIO 仅验证可达；S02 的发布/消费、offset、执行权和恢复尚未实现，031 产品 AC 仍为 0/6，未建立 Acceptance。
...
1. 按 BACKLOG 继续推进 031 S02，冻结 Kafka 发布/消费、数据库 inbox/offset、租约 fencing、检查点及受限调度恢复语义。
...
本轮新增执行唯一 Compose 构建与真实 PostgreSQL/Redis/Kafka、空库初始化、API/Web 健康、同源代理、空 Worker、资源快照和部署态 OpenAPI 检查；随后交付 034、035 与 031 的 S00/S01，在隔离数据库及浏览器验证初始化、会话、CSRF、owner 授权、登录/注销、受保护工作台，以及任务/outbox 原子受理。当前后端 44 tests、前端 13 tests 及适用静态/构建门禁通过。未执行业务消息发布/消费、来源探测、上述计划后续切片、完整 B0 或产品 Acceptance。
