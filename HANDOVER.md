...
**[029 计划](docs/plans/029-时效与数据新鲜度计划.md) S00/S01 完成；S02 API/UI 受控实现已验证，Plan 仍为 in_progress。** 延续现有 `jobs`/`job_attempts` 事实，新增 owner、来源能力及精确配置版本限定的只读新鲜度投影：最近真实尝试、最近完整成功，以及 queued 的限流/预算/瞬时失败/人工重试/内部排队原因；partial 不推进最后成功，延期时长为非负整数微秒。复用现有任务状态 API、OpenAPI 生成链和详情页，无 DDL、新表、依赖、脚本或第二服务。后端本机 357 passed/188 skipped、Ruff/format/mypy，前端 45 tests/lint/typecheck/format/build 通过；运行时 OpenAPI/客户端已复生成。受控 API 响应下，1440×900 与 390×844 详情页字段、刷新点击、无横向溢出、axe 0 violation 通过。首轮远端 backend CI 使用隔离 PostgreSQL 时发现集成夹具试图新增第二 owner，与单 owner schema 约束冲突；现已移除该无效样本，owner 谓词由 SQL 单测覆盖，配置版本/实际聚合由隔离 PostgreSQL 用例验证，修复提交 CI 复验待完成，G4 暂不关闭。未把清理型 fixture 指向 `hotkey-server`，当前库未写入。S03/S04、B0 与产品 AC 仍为 0/6，未建立 Acceptance。
