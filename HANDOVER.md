...
- `app/evidence/` 已实现内部来源访问与在线生命周期控制：owner/source/capability 唯一政策、原子换版、入库前白名单投影；结构化/原始/媒体保留取用户请求与来源上限的更严值，缩期立即作用于已追踪资源；删除或到期后默认拒绝读取，`python -m cli lifecycle cleanup-once` 以 PostgreSQL lease 和有限重试清理 Redis/MinIO 在线副本。`app/backups/` 复用证据 DTO 生成同快照数据库候选归档和 MinIO 引用清单；032 已增加 MinIO 内容流式归档和随机临时前缀回读子片，独立 MinIO 往返通过。隔离 CI run `36091748228` 已用 PostgreSQL 17 + MinIO 真实执行 S02 备份/恢复集成，backend 574 passed/8 skipped（备份/恢复 5 passed），runtime `36091748237` 和 contract `36091748194` 全绿；`CHK-032-G4-002` 技术门禁关闭。首次 CI 中客户端版本与测试表清单问题均已修复。未连接或改动本机唯一 `hotkey-server` 数据库；独立介质、上一完整有效副本晋升、删除/撤权重放、B0 RPO/RTO、S03/S04 和产品 AC 仍未完成。
...
**[032 计划](docs/plans/032-备份与恢复计划.md) S00—S02 技术子片已完成验证，Plan 保持 in_progress。** 维护 CLI 复用 PostgreSQL `pg_dump`/`pg_restore` 与现有 MinIO SDK：历史数据库子片验证同快照表计数/证据引用，当前 schema 的 33 表清单、对象 present/missing/deleted 三态及 `0700/0600` 权限由归档实现维护；新增 MinIO 内容流式归档、版本/ETag 固定及随机临时前缀回读/清理。MinIO 子片 9 项单测、1 项真实随机前缀往返集成通过。backend 隔离 CI run `36091748228`（PostgreSQL 17 + MinIO）574 passed/8 skipped，备份/恢复集成 5 passed；Ruff/format/mypy、runtime `36091748237` 和 contract `36091748194` 全绿，`CHK-032-G4-002` 已关闭。未触碰本机唯一 `hotkey-server` 库。当前源码审查确认 S03 的 `verify-restore` 只在随机临时库验证后删除、删除请求仅写 PostgreSQL，恢复入口不暂停主题；唯一 `hotkey-evidence` bucket 未版本化、无 Object Lock/lifecycle/bucket 加密，且 036 清理适配器拒绝版本化 bucket。维护者已明确要求项目只保留一个固定 MinIO bucket；只读进程核查确认 PostgreSQL 与 MinIO 同属本机故障域（FileVault 开启不改变故障域），锁定的 MinIO SDK `put_object` 不提供条件写入参数。因此同 bucket 专用前缀可作为技术日志位置，但不能称为 WORM 或独立灾备。独立备份路径、最长保留期和密钥恢复责任仍未确认。独立介质、上一完整有效副本晋升、完整 B0 演练、删除/撤权重放、S03/S04 与产品 AC 仍待执行，产品 AC 0/6，未建立 Acceptance。
...
2026-09-25 037/006 X User Read 离线费用子片：官方价格页将 User Read 定义为按返回资源收费，并说明 Developer Console 才是实际费率来源、价格可变；实现接受显式单 User 微美元单价，无默认价。`XApiUserReadCost` 对已知 0/1 与未知结果结算，复用既有 network request + X spend 双预留与报价幂等指纹；不触碰 App/Token、不发 X 请求、不解除 `paid` 核心拒绝。本地 unit+architecture 360 passed、Ruff/format/mypy 通过；隔离 PostgreSQL backend CI 556 passed/12 skipped，contract/runtime 均成功（commit `d3b218b7`）。未触碰本机 `hotkey-server` DB；真实 X 凭据、请求尝试/账期硬上限、付费准入和产品 AC 仍开放。详见 037 EV-037-G3/G4-X04。
