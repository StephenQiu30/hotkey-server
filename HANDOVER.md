...
- `app/evidence/` 已实现内部来源访问与在线生命周期控制：owner/source/capability 唯一政策、原子换版、入库前白名单投影；结构化/原始/媒体保留取用户请求与来源上限的更严值，缩期立即作用于已追踪资源；删除或到期后默认拒绝读取，`python -m cli lifecycle cleanup-once` 以 PostgreSQL lease 和有限重试清理 Redis/MinIO 在线副本。`app/backups/` 复用证据 DTO 生成同快照数据库候选归档和 MinIO 引用清单；032 已增加 MinIO 内容流式归档和随机临时前缀回读子片，独立 MinIO 往返通过。2026-09-25 隔离 CI run `36091137377` 已用 PostgreSQL 17 + MinIO 实际运行完整后端集成：客户端版本、schema 初始化、备份导出、静态门禁通过；全量 pytest 573 passed/8 skipped/1 failed，唯一失败是表清单测试遗漏当前 schema 的 `followed_accounts` 和 `followed_account_aliases`，修正后待 CI 全绿。没有连接或改动本机唯一 `hotkey-server` 数据库，`CHK-032-G4-002` 暂不关闭。已有 X 受控适配器、Firecrawl 网页适配器/显式 CLI 与 `webpage.collect` 业务处理器，当前本地 env 已启用既有 Firecrawl。网页 UI、完整备份恢复/回补和社交平台授权证据仍未完成。
...
**[032 计划](docs/plans/032-备份与恢复计划.md) S00/S01 已完成，Plan 保持 in_progress。** 维护 CLI 复用 PostgreSQL `pg_dump`/`pg_restore` 与现有 MinIO SDK：数据库归档、31 表计数和证据引用基于同一导出快照，清单记录 schema/归档 SHA-256、对象 present/missing/deleted 三态及 `0700/0600` 权限；新增 MinIO 内容流式归档、版本/ETag 固定及随机临时前缀回读/清理。2026-09-25 MinIO 专用单测 9 passed、真实 MinIO 随机前缀往返集成 1 passed。隔离 CI run `36091137377` 已通过 PostgreSQL 17 工具安装/断言、schema 初始化、备份导出、Ruff/format/mypy；全后端 pytest 573 passed/8 skipped/1 failed，唯一失败是 schema 清单断言漏列 `followed_accounts` 和 `followed_account_aliases`，测试预期已同步，等待重跑。未触碰本机唯一 `hotkey-server` 库；`CHK-032-G4-002` 仍开放。没有新增 DDL、依赖、Compose、脚本、HTTP/UI；独立介质、完整恢复/回补、删除重放、B0 与产品 AC 仍为 0/6，未建立 Acceptance。
