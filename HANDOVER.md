...
**[037 计划](docs/plans/037-费用与资源约束计划.md) S00/S01 已完成，Plan 保持 in_progress。** `jobs` 领域已增加免费组件策略、副作用前尝试账本、确定性重试 ID、幂等终态和结果汇总；目标 12 tests、后端全量 75 tests 及静态门禁通过。本切片未接入真实 SDK/HTTP/UI/Worker，不证明 SDK 内部重试或免费业务闭环；S02—S04 及 037 产品 AC 仍为 0/6，未建立 Acceptance。
...
1. 按 BACKLOG 继续推进 037 S02，实现并发预留与耗尽延期；031 S03 留在 M5 长时可靠性阶段继续。
...
本轮新增执行唯一 Compose 构建与真实 PostgreSQL/Redis/Kafka、空库初始化、API/Web 健康、同源代理、空 Worker、资源快照和部署态 OpenAPI 检查；随后交付 034/035/037 S00/S01、031/036 S00—S02，在真实 PostgreSQL、既有 Redis/MinIO/Kafka 及浏览器中验证身份/授权、任务可靠性、生命周期和全尝试计量。当前后端 75 tests、前端 13 tests 及适用静态/构建门禁通过。未执行业务消息处理、真实来源探测、SDK 内部计量、036/037 后续切片、完整 B0 或产品 Acceptance。
