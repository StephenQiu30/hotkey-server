...
describe("KnowledgePage", () => {
  beforeEach(() => {
    ...
    mocks.getKnowledgeDocuments.mockResolvedValue({ data: [{ id: 8, version: 1, revisionNo: 0, type: "report", reportID: 41, vaultPath: "reports/8.md", status: "planned" }] });
    mocks.getKnowledgeProposals.mockResolvedValue({ data: [{ id: 12, version: 1, documentID: 8, baseRevisionNo: 0, status: "pending", reason: "日报发布" }] });
  ...
  it("renders the empty state after the loading state", async () => {
    ...
    mocks.getKnowledgeProposals.mockResolvedValueOnce({ data: [] });
    render(<KnowledgePage />);
    expect(await screen.findByLabelText("正在加载知识投影")).toBeInTheDocument();
    resolveDocuments({ data: [] });
