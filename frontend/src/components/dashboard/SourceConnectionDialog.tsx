      ...
      <DialogContent className="grid max-h-[90vh] grid-rows-[auto_minmax(0,1fr)] gap-0 overflow-hidden p-0 sm:max-w-xl">
        ...
        <form className="grid min-h-0 grid-rows-[minmax(0,1fr)_auto]" onSubmit={submit}>
          <div aria-label="来源连接配置" className="grid min-h-0 gap-5 overflow-y-auto px-6 py-5 sm:grid-cols-2" role="region" tabIndex={0}>
            ...
            {advancedOpen && <Surface id="source-advanced-settings" className="grid gap-4 p-4 sm:col-span-2 sm:grid-cols-2" variant="subtle">
              ...
              <Surface className="bg-background p-4 sm:col-span-2" variant="ring"><p className="text-sm font-medium">证据保留边界</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><label className="flex items-center gap-2 text-sm"><Checkbox checked={form.allowBodyStorage} onCheckedChange={(checked) => updateForm({ allowBodyStorage: checked === true })} />保存正文/摘要</label><label className="flex items-center gap-2 text-sm"><Checkbox checked={form.requiresAttribution} onCheckedChange={(checked) => updateForm({ requiresAttribution: checked === true })} />需要归属标记</label><label className="flex items-center gap-2 text-sm"><Checkbox checked={form.requiresDeletionSync} onCheckedChange={(checked) => updateForm({ requiresDeletionSync: checked === true })} />同步删除</label></div></Surface>
