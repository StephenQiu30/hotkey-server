...
const alertNumberFields = [
  [
    "最低热度",
    "alertHeat",
    MONITOR_LIMITS.alertHeat.min,
    MONITOR_LIMITS.alertHeat.max,
  ],
  [
    "最低动量",
    "alertMomentum",
    MONITOR_LIMITS.alertMomentum.min,
    MONITOR_LIMITS.alertMomentum.max,
  ],
  [
    "最低来源宽度",
    "alertBreadth",
    MONITOR_LIMITS.alertBreadth.min,
    MONITOR_LIMITS.alertBreadth.max,
  ],
  [
    "警告阈值",
    "alertWarning",
    MONITOR_LIMITS.alertSeverity.min,
    MONITOR_LIMITS.alertSeverity.max,
  ],
  [
    "严重阈值",
    "alertCritical",
  ...
  [
    "冷却时间（分钟）",
    "alertCooldown",
    MONITOR_LIMITS.alertCooldown.min,
    MONITOR_LIMITS.alertCooldown.max,
  ],
] as const;

export function MonitorDraftDialog({
  busy,
  form,
  mode,
  onFormChange,
  ...
  open,
  sources,
}: MonitorDraftDialogProps) {
  const availableIds = sources.flatMap((source) =>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="grid h-[90vh] max-h-[90vh] grid-rows-[auto_minmax(0,1fr)] gap-0 overflow-hidden p-0 sm:h-auto sm:max-w-xl">
        <DialogHeader className="border-b border-border px-6 py-5">
          <DialogTitle>
            {mode === "create" ? "新建监控草稿" : "编辑监控草稿"}
          </DialogTitle>
          <DialogDescription>
            配置中英文包含词、别名和排除词，再选择采集节奏与正式来源。
          </DialogDescription>
        </DialogHeader>
        <form
        ...
        >
          ...
          >
            <div className="sm:col-span-2">
              ...
              <Input
                ...
                placeholder="例如：AI 产品发布"
              />
            </div>
            <MonitorRuleEditor
              rules={form.rules}
              onChange={(rules) => onFormChange({ ...form, rules })}
            />
            <div className="sm:col-span-2">
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        onFormChange({
                          ...form,
                          languages: toggleMonitorLanguage(
                            form.languages,
                            value,
                            checked === true
                          ),
            ...
            <div>
              ...
              <Select
                value={form.region}
                onValueChange={(value) =>
                  onFormChange({ ...form, region: value as MonitorRegion })
                }
              >
                <SelectTrigger id="monitor-region" className="mt-2">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={MonitorRegion.China}>中国</SelectItem>
                  <SelectItem value={MonitorRegion.UnitedStates}>
                    美国
                  </SelectItem>
                  <SelectItem value={MonitorRegion.Global}>全球</SelectItem>
                </SelectContent>
              </Select>
            </div>
            {numberFields.map(([label, key, min, max, step]) => (
              <div key={key}>
                ...
                <Input
                  ...
                  value={form[key]}
                  onChange={(event) =>
                    onFormChange({ ...form, [key]: Number(event.target.value) })
                  }
                />
              </div>
            ))}
            <fieldset className="grid gap-4 rounded-md border border-border p-4 sm:col-span-2 sm:grid-cols-2">
              ...
              <p className="text-xs leading-5 text-muted-foreground sm:col-span-2">
                四项门槛需同时达到才触发告警；默认策略偏保守。
              </p>
              {alertNumberFields.map(([label, key, min, max]) => (
                <div key={key}>
                  <Label htmlFor={`monitor-${key}`}>{label}</Label>
                  <Input
                    id={`monitor-${key}`}
              ...
              <label className="flex min-h-9 cursor-pointer items-center gap-2 rounded-md border border-border px-3 text-sm sm:col-span-2">
                <Checkbox
                  aria-label="启用告警邮件"
              ...
              <div className="sm:col-span-2">
                <Label htmlFor="monitor-alert-email-severity">
                  最低邮件级别
                </Label>
                <Select
                  disabled={!form.alertEmailEnabled}
                  value={form.alertEmailMinSeverity}
                  onValueChange={(value) =>
                    onFormChange({
                      ...form,
                      alertEmailMinSeverity: value as "warning" | "critical",
                    })
                  }
                >
                  <SelectTrigger
                    id="monitor-alert-email-severity"
                    className="mt-2"
                  >
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <SelectItem value="warning">警告及严重</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </fieldset>
              ...
              <div className="flex items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    选择已启用来源，最多 {MAX_MONITOR_SOURCES} 个。
                  </p>
                </div>
                ...
                {sources.length > 0 && (
                  <div className="flex items-center gap-3 border-b border-border bg-muted/30 px-3 py-2.5 text-sm">
                    <label className="flex cursor-pointer items-center gap-3 font-medium">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          onFormChange({
                            ...form,
                            sourceIds: checked === true ? selectAllIds : [],
                          })
                        }
                      />
                      <span>全选</span>
                    ...
                    <Button
                      ...
                      onClick={() => onFormChange({ ...form, sourceIds: [] })}
                    >
                      清空
                    </Button>
                  </div>
          ...
          <DialogFooter className="border-t border-border px-6 py-4">
            ...
            <Button
              ...
              disabled={
                busy ||
                !form.rules.some((rule) => rule.value.trim()) ||
                !form.languages.length ||
                !form.sourceIds.length
              }
            >
              {busy && <Loader2 className="animate-spin" />}
              {busy ? "保存中" : mode === "create" ? "创建草稿" : "保存草稿"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
