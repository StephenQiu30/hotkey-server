...
export function ReportWorkspace() {
  ...
  const [form, setForm] = useState({
    type: ReportType.Daily as ReportType,
    timezone: "Asia/Shanghai",
    monitorID: "",
  });

  const loadPage = useCallback(
    async (cursor?: number, pageNumber = 1) => {
      setLoading(true);
      setError(undefined);
      try {
  ...
  const create = async () => {
    setBusy("create");
    try {
      const result = await postReports({
        type: form.type,
      ...
      toast.error(reason instanceof Error ? reason.message : "草稿生成失败");
    } finally {
      setBusy(undefined);
    }
  };
  ...
  const openPreview = async (report: HotKeyAPI.ReportResponse) => {
    ...
    try {
      const result = await postReportsIdPreview({ id: report.id });
      setDetail(result.data?.report);
    } catch (reason) {
      toast.error(reason instanceof Error ? reason.message : "报告预览失败");
      setDetailOpen(false);
    } finally {
  ...
  const mutate = async (
    report: HotKeyAPI.ReportResponse,
    action: "build" | "publish"
  ) => {
    if (report.id == null) return;
    setBusy(report.id);
    try {
      if (action === "build") await postReportsIdBuild({ id: report.id });
      else await postReportsIdPublish({ id: report.id });
      toast.success(
        action === "build" ? "报告草稿已刷新" : "报告已发布并生成交付与知识提案"
      );
      await resetAndLoad();
    } catch (reason) {
    ...
    <section aria-labelledby="reports-title" className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        {canBuild && (
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>生成报告草稿</DialogTitle>
                <DialogDescription>
                  相同类型、监控和周期会刷新同一份草稿。
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-2">
                <div className="space-y-2">
                  ...
                  <Select
                    value={form.type}
                    onValueChange={(value) =>
                      setForm({ ...form, type: value as ReportType })
                    }
                  >
                ...
                <div className="space-y-2">
                  <Label htmlFor="report-monitor">监控 ID（可选）</Label>
                  <Input
                    id="report-monitor"
                    inputMode="numeric"
                    placeholder="全部已启用监控"
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <Select value={type} onValueChange={setType}>
          <SelectTrigger aria-label="报告类型筛选" className="sm:w-40">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={status} onValueChange={setStatus}>
          <SelectTrigger aria-label="报告状态筛选" className="sm:w-40">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
        ...
        <Button variant="outline" onClick={() => void resetAndLoad()}>
          <RefreshCw />
          刷新
        </Button>
      </div>
      {error ? (
        ...
        <Card>
          <Empty className="h-72">
            <EmptyHeader>
              ...
              <EmptyTitle>当前筛选下没有报告</EmptyTitle>
              <EmptyDescription>
                {canBuild
                  ? "生成第一份草稿，系统会固定周期内的事件证据。"
                  : "编辑者生成报告后会出现在这里。"}
              </EmptyDescription>
            </EmptyHeader>
          </Empty>
        ...
        <Card className="overflow-hidden py-0">
          <Table aria-label="报告列表" scrollAreaLabel="报告列表">
            <TableHeader>
              <TableRow>
                <TableHead>报告</TableHead>
                <TableHead className="hidden md:table-cell">周期</TableHead>
                <TableHead>状态</TableHead>
                <TableHead className="hidden lg:table-cell">条目</TableHead>
                ...
                <TableRow key={report.id}>
                  <TableCell>
                    <div className="font-medium">
                      {report.title || `${reportTypeLabel(report.type)}报告`}
                    </div>
                    <div className="mt-1 text-xs text-muted-foreground">
                      {report.monitor_id
                        ? `监控 #${report.monitor_id}`
                        : "全部监控"}{" "}
                      · {report.timezone}
                    </div>
                  </TableCell>
                  <TableCell className="hidden text-xs text-muted-foreground md:table-cell">
                    {formatRadarTime(report.period_start)}
                    <br />至 {formatRadarTime(report.period_end)}
                  </TableCell>
                  <TableCell>
                    <Badge
                      variant={
                  ...
                  <TableCell>
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      {canBuild && report.status === ReportStatus.Draft && (
                        <Button
                          variant="outline"
                          size="sm"
                          disabled={busy === report.id}
                          onClick={() => void mutate(report, "build")}
                        >
                          <RefreshCw />
                          刷新
                        </Button>
                      )}
                      {canPublish && report.status === ReportStatus.Draft && (
                        <Button
                          size="sm"
                          disabled={busy === report.id}
                          onClick={() => void mutate(report, "publish")}
                        >
                          <Send />
                          发布
                        </Button>
          ...
          <CursorPagination
            ...
            onNext={() => {
              ...
              const next = page + 1;
              setCursors((items) => [...items.slice(0, page), nextCursor]);
              void loadPage(nextCursor, next);
            }}
            onPageSizeChange={setPageSize}
            onPrevious={() => {
              if (page > 1) void loadPage(cursors[page - 2], page - 1);
            }}
            page={page}
            pageSize={pageSize}
          />
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>{detail?.title ?? "报告预览"}</DialogTitle>
            <DialogDescription>
              {detail
                ? `${reportTypeLabel(detail.type)} · ${formatRadarTime(
                    detail.period_start
                  )} 至 ${formatRadarTime(detail.period_end)}`
                : "正在读取固定快照"}
            </DialogDescription>
          </DialogHeader>
          {detailLoading ? (
            ...
            <div className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  {detail.summary || "当前周期没有符合条件的事件。"}
                </AlertDescription>
              </Alert>
              {detail.items?.length ? (
                detail.items.map((item) => (
                  <Card key={item.event_update_id} className="p-4">
                ...
                <Empty>
                  <EmptyHeader>
                    <EmptyTitle>本周期没有事件快照</EmptyTitle>
                    <EmptyDescription>
                      报告仍可发布为空报，便于维持固定交付节奏。
                    </EmptyDescription>
                  </EmptyHeader>
                </Empty>
              )}
