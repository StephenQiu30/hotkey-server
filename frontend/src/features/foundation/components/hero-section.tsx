...
export function HeroSection() {
  return (
    <section className="relative isolate px-5 pt-20 pb-24 sm:px-8 sm:pt-28 sm:pb-32 xl:px-0 2xl:pt-32">
      <div
        aria-hidden="true"
        className="foundation-grid absolute inset-0 -z-20"
      />
      <div
        aria-hidden="true"
        className="foundation-mesh absolute inset-x-0 inset-y-0 -z-10 mx-auto max-w-7xl rounded-full opacity-70 blur-3xl"
      />
      <div className="mx-auto flex max-w-4xl flex-col items-center text-center">
        <Badge variant="secondary" className="font-mono font-normal">
          FRONTEND FOUNDATION · 2026
        </Badge>
        ...
        <p className="text-muted-foreground mt-6 max-w-2xl text-base leading-7 text-balance sm:text-lg sm:leading-8">
          HotKey 前端基础已经按 Next.js App Router、shadcn/ui 与 Radix UI
          初始化。页面以留白、柔和层级和必要的交互描边构成统一的无边框体验。
        </p>
        <div className="mt-9 flex flex-col gap-3 sm:flex-row">
          ...
          <Button asChild size="lg" variant="secondary">
            <Link href="#principles">了解设计原则</Link>
          </Button>
        </div>
      </div>
    </section>
