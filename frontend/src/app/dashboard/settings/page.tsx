...
type DraftDialogState = { mode: "create" | "edit"; monitor?: HotKeyAPI.MonitorResponse };
type DetailState = { error?: string; history: HotKeyAPI.MonitorConfigResponse[]; loading: boolean; monitor?: HotKeyAPI.MonitorResponse; open: boolean };
...
export default function MonitorsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [monitorResult, sourceResult] = await Promise.all([
        monitorRequest,
        canEdit ? getSourceConnections({ limit: 100 }) : Promise.resolve(undefined),
      ]);
  ...
  const loadMonitorPage = async (cursor: string | undefined, pageNumber: number) => {
    setLoading(true);
    setLoadError(undefined);
    try {
      const result = await getMonitors({ limit: pageSize, ...(cursor ? { cursor } : {}) });
  ...
  const previousPage = () => page > 1 && void loadMonitorPage(cursors[page - 2], page - 1);

  const openCreate = () => {
    setForm({ ...initialForm(), sourceIds: selectAllMonitorSources(sources.flatMap((source) => source.id == null ? [] : [source.id])) });
  ...
  const publish = async () => {
    if (previewTarget?.id == null || previewTarget.draft?.version == null) return;
    setBusyID(previewTarget.id);
    try {
      await postMonitorsIdPublish({ id: previewTarget.id }, { expected_monitor_version: previewTarget.version ?? 0, expected_draft_version: previewTarget.draft.version });
  ...
  const lifecycle = async (monitor: HotKeyAPI.MonitorResponse, action: "pause" | "resume" | "archive" | "restore") => {
    if (monitor.id == null) return;
    setBusyID(monitor.id);
    const body = { expected_monitor_version: monitor.version ?? 0 } as unknown as HotKeyAPI.LifecycleRequest;
    try {
      if (action === "pause") await postMonitorsIdPause({ id: monitor.id }, body);
      else if (action === "resume") await postMonitorsIdResume({ id: monitor.id }, body);
      else if (action === "restore") await postMonitorsIdRestore({ id: monitor.id }, body);
  ...
  const deleteMonitor = async () => {
    ...
    try {
      await deleteMonitorsId({ id: deleteTarget.id }, { expected_monitor_version: deleteTarget.version ?? 0 } as unknown as HotKeyAPI.LifecycleRequest);
  ...
  const openDetail = async (monitor: HotKeyAPI.MonitorResponse) => {
    ...
    try {
      const [monitorResult, historyResult] = await Promise.all([getMonitorsId({ id: monitor.id }), getMonitorsIdVersions({ id: monitor.id })]);
      setDetail({ history: historyResult.data?.items ?? [], loading: false, monitor: monitorResult.data, open: true });
    } catch (reason) {
      setDetail({ error: reason instanceof Error ? reason.message : "监控详情加载失败", history: [], loading: false, monitor, open: true });
    }
  };
  ...
  return (
    <div className="app-page">
      ...
      {!canEdit && <Card className="mt-6 flex-row items-center justify-between gap-4 px-5 py-4"><div><p className="text-sm font-medium">只读监控目录</p><p className="mt-1 text-xs text-muted-foreground">查看者可以检查已发布配置与版本历史，不能修改运行状态。</p></div><Badge variant="secondary">viewer</Badge></Card>}
      {loading ? <div className="flex h-72 items-center justify-center"><Loader2 className="animate-spin text-muted-foreground" /></div> : loadError ? (
        <Card role="alert" className="mt-6 items-center px-6 py-12 text-center"><p className="text-sm font-medium">监控加载失败</p><p className="text-sm text-muted-foreground">{loadError}</p><Button variant="outline" onClick={() => void load()}>重试</Button></Card>
      ) : !monitors.length ? (
        <Card className="mt-6 gap-0 overflow-hidden py-0"><Empty className="h-72"><EmptyHeader><EmptyMedia variant="icon"><Radar /></EmptyMedia><EmptyTitle>还没有热点监控</EmptyTitle><EmptyDescription>{canEdit ? "至少需要一个已启用来源才能创建监控。" : "当前没有可查看的已发布监控。"}</EmptyDescription></EmptyHeader></Empty><CursorPagination hasNext={hasNextCursor(nextCursor)} loading={loading} onNext={nextPage} onPageSizeChange={setPageSize} onPrevious={previousPage} page={page} pageSize={pageSize} /></Card>
      ) : (
        <Card className="mt-6 gap-0 overflow-hidden py-0">
          <div className="hidden grid-cols-[minmax(0,1.3fr)_110px_100px_minmax(240px,1fr)] gap-4 border-b border-border px-5 py-3 text-xs text-muted-foreground lg:grid"><span>监控</span><span>采集间隔</span><span>状态</span><span className="text-right">操作</span></div>
          <div className="divide-y divide-border">{monitors.map((monitor) => {
            const config = monitor.draft ?? monitor.published;
            const rule = config?.rules?.[0];
            const busy = busyID === monitor.id;
            return <div key={monitor.id} className="grid gap-3 px-4 py-4 lg:grid-cols-[minmax(0,1.3fr)_110px_100px_minmax(240px,1fr)] lg:items-center lg:gap-4 lg:px-5">
              <div className="min-w-0"><div className="flex items-center gap-2"><Search className="h-3.5 w-3.5 text-muted-foreground" /><p className="truncate text-sm font-medium">{monitor.name || `监控 #${monitor.id}`}</p></div><p className="mono mt-2 truncate text-xs text-muted-foreground">{rule?.value || monitor.description || "暂无规则"}</p></div>
              <span className="mono text-xs text-muted-foreground">{config?.collection_interval_seconds ? `${config.collection_interval_seconds}s` : "—"}</span>
              <Badge variant="outline" className="w-fit">{monitorStatusLabel(monitor.status)}</Badge>
              <div className="flex flex-wrap justify-start gap-2 lg:justify-end">
                ...
                {canAdmin && monitor.status === MonitorStatus.Archived && <><Button variant="outline" size="sm" className="gap-1.5" disabled={busy} onClick={() => void lifecycle(monitor, "restore")}><RotateCcw />恢复</Button><Button variant="ghost" size="sm" className="gap-1.5 text-destructive hover:text-destructive" disabled={busy} onClick={() => setDeleteTarget(monitor)}><Trash2 />删除</Button></>}
              </div>
            </div>;
          })}</div>
          <CursorPagination hasNext={hasNextCursor(nextCursor)} loading={loading} onNext={nextPage} onPageSizeChange={setPageSize} onPrevious={previousPage} page={page} pageSize={pageSize} />
        </Card>
      )}
      ...
      <ConfirmDeleteDialog open={deleteTarget != null} onOpenChange={(open) => !open && setDeleteTarget(undefined)} title="删除监控" description="监控将从工作区隐藏，已采集内容、历史事件、报告和审计记录仍会保留。" resourceName={deleteTarget?.name || `监控 #${deleteTarget?.id ?? ""}`} onConfirm={deleteMonitor} loading={busyID === deleteTarget?.id} />
    </div>
  );
}
