  ...
  return <PageShell>
    ...
    <section className="mt-10" aria-labelledby="retention-title">
      <Card className="overflow-hidden">
        <CardHeader className="flex flex-row flex-wrap items-center gap-3 bg-secondary/30"><div><CardTitle id="retention-title" className="flex items-center gap-2"><Archive className="h-4 w-4" />数据保留</CardTitle><p className="mt-2 text-sm text-muted-foreground">先生成固定候选清单与 Hash，明确批准后执行；每次最多处理 1000 条。</p></div>
    ...
    <section className="mt-10" aria-labelledby="audit-title">
      <Card className="overflow-hidden"><CardHeader className="bg-secondary/30"><CardTitle id="audit-title" className="flex items-center gap-2"><ScrollText className="h-4 w-4" />审计记录</CardTitle>
        ...
        <CardContent className="flex items-center justify-between bg-secondary/25 py-4"><p className="text-xs text-muted-foreground">第 {auditPage} 页</p><div className="flex gap-2"><Button size="sm" variant="outline" disabled={auditPage === 1 || auditLoading} onClick={() => void loadAudit(cursorHistory[auditPage - 2], auditPage - 1)}>上一页</Button><Button size="sm" variant="outline" disabled={!nextCursor || auditLoading} onClick={() => { if (!nextCursor) return; setCursorHistory((items) => [...items.slice(0, auditPage), nextCursor]); void loadAudit(nextCursor, auditPage + 1); }}>下一页</Button></div></CardContent>
      </Card>
    </section>
