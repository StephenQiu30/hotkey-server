...
if [ -n "$db_refs" ]; then
  invalid_db_refs=$(printf '%s\n' "$db_refs" | grep -Ev '^(internal/database/|internal/app/|internal/collector/|internal/aggregator/|internal/cleanup/|internal/repository/|internal/platform/database/|internal/fxapp/|internal/module/|internal/worker/)' || true)
...
if [ -n "$env_refs" ]; then
  invalid_env_refs=$(printf '%s\n' "$env_refs" | grep -Ev '^(internal/config/|internal/app/|internal/fxapp/|internal/platform/database/bootstrap\.go:)' || true)
