...
Bounded source probes, persistent page collection foundations, manual event grouping, platform trends, notifications, frozen comment samples, manual viewpoint analysis, and cited knowledge snapshots are implemented. Knowledge supports exact search and an explicit recoverable semantic index through self-hosted Ollama and pgvector. **The existing MinIO instance and real sources are not admitted yet; real-content evaluation, automatic model labeling, controlled statistics, and cited Q&A remain open.** Saving a draft does not start collection. Diagnostic jobs do not produce social data.
