...
Bounded source probes, persistent page collection foundations, manual event grouping, platform trends, notifications, frozen comment samples, manual viewpoint analysis, and cited knowledge snapshots are implemented. Knowledge supports exact and semantic search, controlled comment statistics, deterministic cited answers, and an online withdrawal barrier; the recoverable semantic index uses self-hosted Ollama and pgvector. A synthetic lifecycle now covers withdrawal manifests, conservative shared-page removal, exact-key MinIO version deletion, and post-restore replay. **The existing MinIO instance and real sources are not admitted yet; real-content evaluation, automatic model labeling, real backup restore, and old-application rollback remain open.** Saving a draft does not start collection. Diagnostic jobs do not produce social data.
...
Operators can export a private withdrawal manifest with `python -m cli deletion-export <path>`, replay it after an isolated restore with `deletion-replay <path>`, and run bounded object cleanup with `evidence-reconcile --limit 20`. Cleanup requires complete `HOTKEY_S3_*` settings. The application does not create buckets or change object-lock and lifecycle policy; production deletion remains unverified until the existing instance's TLS path, delete permissions, version history, and incomplete multipart lifecycle are checked.
