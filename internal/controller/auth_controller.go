...
// @Accept json
// @Produce json
// @Param body body dto.EmailRegisterRequest true "Verified registration payload"
...
func registerHandler(svc *service.AuthService, cookieCfg refreshCookieConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := c.ShouldBindJSON(&body); err != nil {
			log.Printf("[auth] register: body validation failed: %v", err)
			platformhttp.RespondError(c, enum.ErrorCodeAuthInvalidInput, "")
			return
		}
		...
		if err != nil {
			_ = c.Error(service.PublicError(err))
			return
		}
