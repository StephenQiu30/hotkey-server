...
func NewWeeklyEmailScheduler(producer Producer, opts WeeklyEmailOptions) *WeeklyEmailScheduler {
	...
	return &WeeklyEmailScheduler{
		...
		reportIDProvider:    opts.ReportIDProvider,
		weeklySendDay:       opts.WeeklySendDay,
		defaultWeeklySendAt: sendAt,
...
func (s *WeeklyEmailScheduler) Tick(ctx context.Context) error {
	...
	reportID := s.reportID
	if reportID == "" && s.reportIDProvider != nil {
		reportID = s.reportIDProvider()
	}
	if reportID == "" {
		return nil
	}
	...
	for _, recipient := range s.recipients {
		...
		if _, err := s.producer.Enqueue(ctx, queue.EnqueueRequest{
			...
			Payload:        payload,
			IdempotencyKey: fmt.Sprintf("send_weekly_email:%s:%s:%s", reportID, recipient.UserID, weekOf),
		}); err != nil {
			return err
		}
	}
