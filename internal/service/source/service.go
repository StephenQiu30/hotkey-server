...
func buildSource(existing Source, name string, sourceType SourceType, rawURL string, complianceNote string, fetchIntervalMin int, rateLimitPerHour int, channelIDs []string) (Source, error) {
	...
	if (sourceType == SourceTypePublicPage || sourceType == SourceTypeWeChatMP) && complianceNote == "" {
...
func validSourceType(sourceType SourceType) bool {
	return sourceType == SourceTypeRSS || sourceType == SourceTypePublicPage || sourceType == SourceTypeHackerNews || sourceType == SourceTypeWeChatMP || sourceType == SourceTypeZhihu
}
