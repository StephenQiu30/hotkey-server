...
FROM contents c
JOIN content_embeddings ce ON ce.content_id = c.id AND ce.active
JOIN event_embeddings ee ON ee.model_profile_id = ce.model_profile_id AND ee.model_version = ce.model_version AND ee.active
JOIN monitor_matches mm ON mm.content_id = c.id AND mm.decision = 'accepted'
JOIN monitor_events me ON me.monitor_id = mm.monitor_id AND me.event_id = ee.event_id
JOIN events e ON e.id = ee.event_id
WHERE c.id = $1 AND c.content_status = 'active'
  AND e.lifecycle_status IN ('detected','active','cooling','closed') AND e.deleted_at IS NULL
ORDER BY ce.embedding <=> ee.embedding, e.event_key ASC LIMIT $2`, contentID, limit)
	if err != nil {
