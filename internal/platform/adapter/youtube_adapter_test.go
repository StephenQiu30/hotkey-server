...
// searchResponseJSON returns a YouTube /search endpoint response with the given video IDs.
...
// videoDetailJSON returns a YouTube /videos endpoint response.
// The videos endpoint returns id as a plain string.
...
// videoDetailWithStatusJSON returns a /videos response with explicit status (for filter tests).
...
// videoDetailWithCaptionsJSON returns a /videos response including captions.
...
// routeYouTubeMock creates a mock server that routes /search and /videos to different responses.
