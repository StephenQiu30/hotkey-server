CREATE TABLE IF NOT EXISTS authorizations (
    ...
    platform VARCHAR(32) NOT NULL CHECK (platform IN ('x', 'weibo', 'zhihu', 'xiaohongshu', 'youtube', 'bilibili', 'reddit', 'hackernews', 'git', 'wechat_mp')),
    platform_user_id VARCHAR(255) NOT NULL DEFAULT '',
    display_name VARCHAR(255) NOT NULL DEFAULT '',
    access_token_enc TEXT NOT NULL,
    refresh_token_enc TEXT NOT NULL DEFAULT '',
    status VARCHAR(32) NOT NULL DEFAULT 'connected' CHECK (status IN ('connected', 'expired', 'revoked', 'error')),
