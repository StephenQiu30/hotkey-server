...
- `server` 作为后端唯一运行入口；历史约定或旧分支中的 `apps/` 运行时目录与运行代码在当前阶段一律移除，不保留。
...
- 任何有提交动作的工作单元（Issue/PR/单文件变更）必须按 test-first 顺序执行：先提交 `test:`，再提交 `impl:`，必要时再提交 `refactor:`/`chore:`；非功能类变更可直接 `docs:`，但需写明原因和后续补测计划。
- 文档更新、API 更新、代码提交应同步进行，不出现“先写代码后补文档”。
- 每次实现改动必须先补测试并执行测试通过后再提交，提交前后都必须确认 `git status --short` 为空，确保工作区无中间产物。
