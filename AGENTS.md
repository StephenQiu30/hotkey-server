...
- 本地网页/浏览器采集按 [047 Design](docs/design/047-本地网页与浏览器采集设计.md) 与 [047 Plan](docs/plans/047-本地网页与浏览器采集计划.md) 分片推进；S00 与 S01 网页契约/默认关闭适配器基础已接受，服务恢复、真实探针和业务闭环仍未完成。后续先登记来源专属能力、连接版本和页级事务，再注册业务 Worker；不得把工具可用当作平台接入成功，或把 Firecrawl 内置渲染器当作完整交互服务。外采适配器保持无 ORM，业务编排归 content，执行权和预算归 jobs；不新增第二套队列或任务数据库。跨仓库 Firecrawl 修复单独检查差异，不混入 HotKey 提交。
