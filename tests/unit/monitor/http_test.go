...
func setupTestHandler() (*monitor.Handler, *fakemonitor.Repo) {
	repo := &fakemonitor.Repo{NextID: 1}
	svc := monitor.NewService(repo)
	return monitor.NewHandler(svc), repo
}
...
func requestWithUser(method, path string, body string, userID int64) *http.Request {
	...
	return req.WithContext(monitor.ContextWithUserID(context.Background(), userID))
}
