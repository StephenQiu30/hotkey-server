...
// ValidateHTTPTransport rejects direct Gin response writes from module HTTP transports.
func ValidateHTTPTransport(root string) error {
	...
	if err != nil {
		return fmt.Errorf("inspect module HTTP transports: %w", err)
	}
	if len(violations) == 0 {
		return nil
	}
	...
	for _, found := range violations {
		fmt.Fprintf(&message, "%s:%d:%d direct Gin response output via Context.%s is forbidden\n", found.path, found.line, found.column, found.method)
	}
