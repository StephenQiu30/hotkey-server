...
func TestReactivateByUniqueKeyPreservesAttemptHistory(t *testing.T) {
	...
	defer runtime.Close()
	if err := database.InitializeEmpty(ctx, runtime.Pool); err != nil { t.Fatal(err) }
	store := NewStore(runtime)
	...
	if err != nil { t.Fatal(err) }
	if _, err := runtime.SQL.ExecContext(ctx, `UPDATE river_job SET state = 'discarded', attempt = 2, finalized_at = now() WHERE id = $1`, id); err != nil { t.Fatal(err) }
	if _, err := runtime.SQL.ExecContext(ctx, `INSERT INTO river_job_attempt (job_id, attempt, error) VALUES ($1, 1, 'first'), ($1, 2, 'second')`, id); err != nil { t.Fatal(err) }

	activatedID, err := store.ReactivateByUniqueKey(ctx, job.Kind, job.UniqueKey)
	if err != nil || activatedID != id { t.Fatalf("ReactivateByUniqueKey() = %d/%v", activatedID, err) }
	var state string
	var attempt, maxAttempts, history int
	var finalized *time.Time
	if err := runtime.SQL.QueryRowContext(ctx, `SELECT state, attempt, max_attempts, finalized_at FROM river_job WHERE id = $1`, id).Scan(&state, &attempt, &maxAttempts, &finalized); err != nil { t.Fatal(err) }
	if err := runtime.SQL.QueryRowContext(ctx, `SELECT count(*) FROM river_job_attempt WHERE job_id = $1`, id).Scan(&history); err != nil { t.Fatal(err) }
...
func TestReactivateByUniqueKeyRejectsUnsafeStatesAndRollsBack(t *testing.T) {
	...
	for _, state := range []string{"running", "completed"} {
		if _, err := runtime.SQL.ExecContext(ctx, `UPDATE river_job SET state = $1 WHERE id = $2`, state, id); err != nil { t.Fatal(err) }
	...
	if _, err := runtime.SQL.ExecContext(ctx, `UPDATE river_job SET state = 'cancelled', finalized_at = now() WHERE id = $1`, id); err != nil { t.Fatal(err) }
	err = runtime.WithinTransaction(ctx, func(transactionCtx context.Context, _ database.Transaction) error {
		if _, err := store.ReactivateByUniqueKey(transactionCtx, job.Kind, job.UniqueKey); err != nil { return err }
		return context.Canceled
	})
	if !errors.Is(err, context.Canceled) { t.Fatalf("rollback error = %v", err) }
	var state string
	if err := runtime.SQL.QueryRowContext(ctx, `SELECT state FROM river_job WHERE id = $1`, id).Scan(&state); err != nil { t.Fatal(err) }
	if state != "cancelled" { t.Fatalf("rolled back state = %q", state) }
}
