...
已完成并有长期验收证据的交付是：模块化单体与工程门禁、单一 Schema/数据库平台、统一 HTTP 契约与观测基础、身份认证/会话/权限、版本化 Monitor 与 Source 配置、查询规划与 RSS/Atom/Hacker News 的共享采集运行，以及 Content 标准化、确定性去重、MinIO 文本证据、删除同步和安全 Content 查询（PRD/Plan 001–007）。对应证据见 [Acceptance 索引](docs/acceptance/README.md)。

事件、热度、AI、知识、报告和投递仍是后续目标；治理任务 018 是已审核的非阻塞 `ready` 支线，PRD/Plan 006 与 007 已归档为 `archived` / `done`，且 [Acceptance-007](docs/acceptance/007-内容标准化去重与MinIO证据验收.md) 为 `accepted`；PLAN-008 已独立复审为 `accepted/approved/ready`，是当前可开工的 AI 基础任务，但尚未实现；009–017 仍处于 `review` / `backlog`。不得把本节“目标能力”描述为已上线能力。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
