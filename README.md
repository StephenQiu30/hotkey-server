...
事件、热度、AI 应用任务、知识、报告和投递仍是后续目标；治理任务 018 是已审核的非阻塞 `ready` 支线，PRD/Plan 006、007 与 008 已归档为 `archived` / `done`，且 [Acceptance-007](docs/acceptance/007-内容标准化去重与MinIO证据验收.md) 与 [Acceptance-008](docs/acceptance/008-AIProvider与Embedding基础验收.md) 为 `accepted`。PLAN-008 已交付的是安全的 AI 基础与控制面，不代表已配置生产模型或已上线后续 AI 工作流；PLAN-009 已通过开工审核，Task 1–2 已验证并获独立复核，下一步是 Task 3（`accepted` / `in_progress`），010–017 仍处于 `review` / `backlog`。不得把本节“目标能力”描述为已上线能力。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
