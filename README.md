...
个人热点监控学习项目。服务端已统一为 **Python / FastAPI / SQLAlchemy / RabbitMQ**，采用 PostgreSQL、Alembic、Celery；浏览器端使用 Next.js App Router、React 和 TypeScript。业务API统一位于`/api/*`，URL不携带数字版本段，不保留旧路径兼容层。
...
需要 Docker Compose v2.24.4+。本地开发从仓库根目录执行：
...
账号初始化会交互读取至少 12 位密码，无默认账号。打开 http://localhost:8010。Next.js 开发服务器会热更新前端；后端源码同步后 Uvicorn 自动重载，Worker 和 Scheduler 同步后重启。迁移仍需显式执行，应用启动不会自动改表。开发端口仅绑定本机回环地址，可复制 `.env.example` 修改端口；改变网页端口时也要修改允许来源。
...
- `frontend/src/app/`：Next.js App Router 和工作台客户端边界；`src/api/` 为自动生成端点，`src/request.ts` 为 Axios 传输封装。品牌 Logo 母版为 `frontend/public/logo.svg`。
...
- 根 Compose 是唯一服务拓扑：`docker-compose.yml` 提供本地开发配置，`docker-compose-env.yml` 提供测试/预发环境覆盖，`docker-compose-prod.yml` 提供线上覆盖。环境与线上部署通过 `-f` 叠加，不维护第二套服务定义；详细命令见 [运行手册](docs/operations/006-Python运行与验证.md)。
...
本地 Web 服务通过根 Compose 启动，地址为 `http://localhost:8010`；单独调试时运行 `npm run dev --prefix frontend`。登录工作台当前设置为 `noindex`；公开首页、可公开事件页及对应 sitemap 会在完成产品设计与访问权限评审后实现。
