...
事件、热度、知识、报告和投递仍是后续目标；治理任务 018 是已审核的非阻塞 `ready` 支线，PRD/Plan 006、007、008 与 009 已归档为 `archived` / `done`，且对应 Acceptance 均为 `accepted`。PLAN-009 在 `d4efda5` 基线通过完整 `make ci`、PostgreSQL/Redis/MinIO integration race，并经非主要编写者 APPROVED；010–017 仍处于 `review` / `backlog`，必须各自完成开工门禁。不得把本节“目标能力”描述为已上线能力。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
