...
事件治理主线 010 已实现 Event 查询、生命周期、合并/拆分、成员锁、候选决策审计和版本化热度快照，并通过本地 `make ci` 与事件/数据库 integration race。011–017 已陆续落地热度 API、证据化 Claim API、事务入队、知识文档/提案持久化、冻结报告的查询/预览/管理员发布 API、可自助管理且可轮换私有 Token 的 RSS/邮件订阅、投递幂等持久化和保留清理适配器；这些任务仍在补齐完整 Worker、对账、Vault/邮件外部投递、故障和独立 Acceptance，不能描述为已上线。治理任务 018 是已审核的非阻塞 `ready` 支线，006–009 已归档为 `archived` / `done`。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
