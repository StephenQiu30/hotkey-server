...
新增 Bluesky 查询预览和有界 CLI 来源探测，可读取公开线程并区分受限、空结果和部分结果。持久采集账本、单页消费者、MinIO适配器、UTC日预算、周期槽、运行列表和只读收件箱基础已经实现；B站搜索引用可依次派生正文详情、一页根评论和一页回复，并分别保留证据。工作台已支持人工事件归并/拆分、分平台趋势、站内提醒、冻结评论样本、人工观点分析和带引用的知识快照。知识库提供精确检索，并可通过自建 Ollama 与 pgvector 建立可恢复语义索引。**现有MinIO与真实来源尚未准入，多页/多根评论展开、真实内容盲测、自动模型标注、受控统计与引用问答尚未实现。** 草稿和页面读取不会启动采集，诊断不会生成社交数据。当前实现进度见 [007 Plan](docs/plans/007-热点事件与评论知识库计划.md)。
