...
目前可以初始化单用户账号、登录和退出、保存/编辑多来源关键词监控草稿、提交/查询/取消诊断任务。诊断经 PostgreSQL Outbox → RabbitMQ → Celery prefork → PostgreSQL 结果表，支持幂等、租约、重投与过期执行拒绝。工作台仅在页面可见且存在排队或运行中的任务时，每2秒读取一次生成的Job与CollectionRun接口；页面隐藏会停止并取消在途轮询，两类活动项都进入终态后完整刷新一次并停止。
