...
已完成并有长期验收证据的交付是：模块化单体与工程门禁、单一 Schema/数据库平台、统一 HTTP 契约与观测基础、身份认证/会话/权限、版本化 Monitor 与 Source 配置、查询规划与 RSS/Atom/Hacker News 的共享采集运行、Content 标准化/确定性去重/MinIO 文本证据/删除同步/安全 Content 查询，以及 AI Model Profile、Provider 运行复用/预算、1024 维 Embedding 存储检索和管理员控制面（PRD/Plan 001–008）。对应证据见 [Acceptance 索引](docs/acceptance/README.md)。

事件、热度、知识、报告和投递仍是后续目标；治理任务 018 是已审核的非阻塞 `ready` 支线，PRD/Plan 006、007 与 008 已归档为 `archived` / `done`，且 [Acceptance-007](docs/acceptance/007-内容标准化去重与MinIO证据验收.md) 与 [Acceptance-008](docs/acceptance/008-AIProvider与Embedding基础验收.md) 为 `accepted`。PLAN-008 已交付的是安全的 AI 基础与控制面，不代表已配置生产模型或已上线后续 AI 工作流；PLAN-009 的 Task 1–6、两项 P1 整改和 relevance-review 序列化修复已在 `d4efda5` 基线通过完整 `make ci` 与 PostgreSQL/Redis/MinIO integration race，当前只剩非主要编写者复审，仍为 `accepted` / `in_progress`。010–017 仍处于 `review` / `backlog`。不得把本节“目标能力”描述为已上线能力。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
