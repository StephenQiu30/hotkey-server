...
已完成并有长期验收证据的交付是：模块化单体与工程门禁、单一 Schema/数据库平台、统一 HTTP 契约与观测基础、身份认证/会话/权限，以及版本化 Monitor 与 Source 配置（PRD/Plan 001–005）。对应证据见 [Acceptance 索引](docs/acceptance/README.md)。

查询规划、采集、事件、热度、AI、知识、报告和投递仍是后续目标；治理任务 018 是已审核的非阻塞 `ready` 支线，PRD/Plan 006 已通过独立审核并处于 `accepted` / `in_progress`，007–017 仍处于 `review` / `backlog`，不得把本节“目标能力”描述为已上线能力。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
