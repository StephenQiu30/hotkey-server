...
Content 标准化、去重、MinIO、事件、热度、AI、知识、报告和投递仍是后续目标；治理任务 018 是已审核的非阻塞 `ready` 支线，PRD/Plan 006 已归档为 `archived` / `done`，PRD/Plan 007 已完成独立复核并处于 `accepted` / `in_progress`，尚未完成验收；008–017 仍处于 `review` / `backlog`。不得把本节“目标能力”描述为已上线能力。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
