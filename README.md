...
事件治理主线 010 与热度趋势主线 011 已完成并归档：涵盖 Event 查询、生命周期、合并/拆分、成员锁、候选决策审计、版本化热度快照、趋势和 Monitor 排序，并通过本地 `make ci` 与事件/数据库 integration race。PLAN-012 正在实现证据化摘要、实体与主张闭环；PLAN-013–017 仍为 `backlog`，不得描述为已上线或已验收。治理任务 018 是已审核的非阻塞 `ready` 支线。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
