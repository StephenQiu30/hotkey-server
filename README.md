...
事件、热度、AI、知识、报告和投递仍是后续目标；治理任务 018 是已审核的非阻塞 `ready` 支线，PRD/Plan 006 与 007 已归档为 `archived` / `done`，且 [Acceptance-007](docs/acceptance/007-内容标准化去重与MinIO证据验收.md) 为 `accepted`；PLAN-008 已独立复审并正在实现，009–017 仍处于 `review` / `backlog`。不得把本节“目标能力”描述为已上线能力。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
