...
新增 Bluesky 查询预览和有界 CLI 来源探测，可读取公开线程并区分受限、空结果和部分结果。持久采集账本、单页消费者、MinIO适配器、UTC日预算、周期槽、运行列表和只读收件箱基础已经实现；B站搜索引用可依次派生一个正文详情、一页最多20条根评论和其中一条根评论的一页最多20条回复，四页分别留存证据，评论与回复通过同来源根帖继承监控上下文。来源准入已按 `source.operation` 精确建模。工作台已支持人工创建事件、把收件箱内容加入或移出事件、合并及按成员拆分事件，并保存可关联回查的不可变修订快照。**现有MinIO与真实来源尚未准入，多页/多根评论展开、趋势提醒、观点分析与知识问答尚未实现。** 草稿和页面读取不会启动采集，诊断不会生成社交数据。当前实现进度见 [007 Plan](docs/plans/007-热点事件与评论知识库计划.md)。
