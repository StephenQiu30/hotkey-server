...
事件治理主线 010、热度趋势主线 011 与证据化事件智能主线 012 已完成并归档：涵盖 Event 查询、生命周期、合并/拆分、成员锁、版本化热度快照、趋势和 Monitor 排序，以及证据绑定的摘要、实体和主张、受限重生成 API、Provider 降级与离线评测。全部通过本地 `make ci`；详细证据见 [Acceptance-012](docs/acceptance/012-证据化事件摘要实体与主张验收.md)。PLAN-013–017 仍为 `backlog`，不得描述为已上线或已验收。治理任务 018 是已审核的非阻塞 `ready` 支线。唯一的任务状态事实源是 [PRD 索引](docs/prd/README.md) 和 [Plan 索引](docs/plans/README.md)；设计边界见 [设计索引](docs/design/README.md)。
...
所有测试源码统一放在 [`tests/`](tests/)；业务目录不提交 `*_test.go`。由于 Go 的同包测试必须在被测包目录编译，`make test` 会在运行期间将 `tests/_suite/` 的测试文件临时映射到对应包，结束时自动清理。需要只运行一个业务包时，使用同一入口，例如：
