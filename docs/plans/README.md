...
| 010 | [事件聚类生命周期与人工治理](../prd/010-事件聚类生命周期与人工治理.md) | [执行计划](010-事件聚类生命周期与人工治理计划.md) | 009 | in_progress | pending |
...
PLAN-008/009 已归档为 `done/approved`。PLAN-010 的事件生命周期、治理事务和 API 已实现并通过事件/数据库集成门禁；canonical 合并版本契约修订后，当前等待独立复审，不能继续实施或归档。011–017 虽已有部分核心契约，但尚未通过各自的 Plan Review 和依赖准入，必须保持 `backlog`，不能标记为正在实施或 done。
