...
PLAN-008/009 已归档为 `done/approved`。PLAN-010 的事件生命周期、治理事务和 API 已实现并通过事件/数据库集成门禁，当前等待 Acceptance-010 与实现复核；011–017 虽已有部分核心契约，但尚未通过各自的 Plan Review 和依赖准入，必须保持 `backlog`，不能标记为正在实施或 done。
