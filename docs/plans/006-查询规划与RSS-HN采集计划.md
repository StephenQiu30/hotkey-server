...
| 创建 | internal/modules/source/domain/checkpoint.go | 检查点契约 |
...
| 创建 | internal/modules/source/infrastructure/postgres/run_repository.go | 运行与检查点持久化 |
| 创建 | internal/modules/source/transport/http/runs.go | 管理查询与安全重试 |
| 修改 | db/schema.sql | checkpoints、collection_runs、run_items |
...
1. 先写 Connector 契约、查询签名、条件请求和检查点红灯测试。
2. 实现 SourceItem 与查询规划，不接入下游业务。
3. 实现 RSS/Atom 的 ETag、Last-Modified 和分页。
4. 实现 HN 增量游标、超时和限流处理。
5. 持久化运行项，只有完整对账后推进检查点。
