...
owner: HotKey Team
phase: P0
canonical_path: docs/plans/028-可观测性部署与质量门禁计划.md
...
1. 将 AC-028-* 转为领域、应用、Transport、前端组件和浏览器场景测试。
...
- [ ] Slice-028-1：实现「健康、指标、日志、追踪和运行总览」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-028-* 回归。
- [ ] Slice-028-2：实现「开发/生产容器部署」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-028-* 回归。
- [ ] Slice-028-3：实现「CI、契约、浏览器和安全门禁」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-028-* 回归。
- [ ] Slice-028-4：实现「备份恢复、发布回滚和容量基线」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-028-* 回归。
