...
3. `TASK-001-S01-T03`：对当前身份角色、页面菜单、服务端 RBAC 和直接 API 调用建立权限差距夹具；先证明 Analyst 差距，不先放宽权限。映射 `AC-001-002`、`AC-001-004`。
...
- [ ] `CHK-001-G1-001`（`AC-001-002`）：四角色权限矩阵已由产品与后端共同评审，Analyst 差距和迁移路径显式；预期证据：权限矩阵、负向 API 测试计划。
