...
owner: HotKey Team
phase: P0
canonical_path: docs/plans/archive/002-Vercel风格与shadcn-ui设计系统计划.md
...
- [x] Slice-002-1：实现「颜色、字体、间距、圆角和阴影令牌」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-002-* 回归。
- [x] Slice-002-2：实现「导航、表单、表格、筛选、对话框、空态和反馈组合规范」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-002-* 回归。
- [x] Slice-002-3：实现「桌面/平板/移动断点」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-002-* 回归。
- [x] Slice-002-4：实现「正常、加载、空、错误、权限不足状态」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-002-* 回归。
