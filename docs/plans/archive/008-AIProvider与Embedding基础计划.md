...
canonical_path: docs/plans/archive/008-AIProvider与Embedding基础计划.md
status: archived
...
- `db/schema.sql` 是唯一执行 Schema。既有库升级仅使用 `docs/operations/003-AIProvider与Embedding升级.md`，先备份和演练；绝不运行时 DDL、`DROP SCHEMA` 或不受控 reset。
...
| 创建/修改 | `docs/operations/003-AIProvider与Embedding升级.md`, `docs/acceptance/archive/008-AIProvider与Embedding基础验收.md` | 可演练升级/回退与最终长期证据。 |
...
- Create: `docs/operations/003-AIProvider与Embedding升级.md`
...
- [x] **Step 2: Run RED.**
  ...
  Run: `go test ./internal/platform/config ./internal/shared/errors ./internal/platform/database ./internal/platform/database/model ./test/architecture -count=1`
...
- [x] **Step 5: Run GREEN.**
  ...
  Run: `HOTKEY_TEST_DSN='postgres:///hotkey_plan008_test?sslmode=disable' go test -tags=integration ./internal/platform/database ./test/architecture -count=1`
...
- [x] **Step 6: Commit.**
  ...
  git add go.mod go.sum .env.example internal/platform/config internal/shared/errors \
    ...
    docs/operations/003-AIProvider与Embedding升级.md test/architecture
...
- [x] **Step 2: Run RED.**
  ...
  Run: `HOTKEY_TEST_DSN='postgres:///hotkey_plan008_test?sslmode=disable' go test -tags=integration ./internal/platform/database ./internal/platform/database/model ./test/architecture ./internal/modules/intelligence/application -count=1`
...
- [x] **Step 4: Run GREEN.**
  ...
  Run: `HOTKEY_TEST_DSN='postgres:///hotkey_plan008_test?sslmode=disable' go test -race -tags=integration ./internal/modules/intelligence/application ./internal/bootstrap ./internal/modules/intelligence/infrastructure/postgres ./internal/platform/database ./internal/platform/database/model ./test/architecture -count=1 && go test ./internal/modules/intelligence/infrastructure/provider -count=1`
...
- [x] **Step 5: Commit.**
  ...
  git add internal/modules/intelligence/application internal/modules/intelligence/domain/provider.go \
    ...
    docs/operations/003-AIProvider与Embedding升级.md internal/bootstrap/app.go
...
- Modify: `internal/bootstrap/{app,app_test}.go`, `internal/modules/intelligence/{application/model_profile_service.go,domain/profile.go,infrastructure/postgres/{repository,model_profile_repository,model_profile_repository_test}.go}`, `test/architecture/openapi_test.go`, `docs/openapi/swagger.json`
...
- [x] **Step 4: Obtain independent final review and archive.** A non-author reviews code, docs, upgrade/rollback, SDK fixture, security boundaries, concurrency tests, OpenAPI and final worktree. Only an `APPROVED` review changes Acceptance result to `accepted`, PRD/Plan to `archived/done`, and indexes/README from 008 `review/backlog` to complete. PLAN-009 readiness is a separate gate.

- [x] **Step 5: Commit.**

  ```bash
  git add docs/acceptance/archive/008-AIProvider与Embedding基础验收.md docs/operations/README.md \
...
| v1.21 | 2026-07-17 | Task 6 已完成实现与 GREEN：管理员专用 profile CRUD/restore、显式 409 乐观锁、write-only credential 边界、Fx 路由、六路径 OpenAPI 及 PostgreSQL 集成和安全契约测试均已覆盖；整体仍等待 Task 7。 |
| v1.22 | 2026-07-17 | Task 7 已完成：Acceptance-008 保存升级/回退、HNSW、Provider fixture、ONNX 默认降级、完整 CI/race 和 APPROVED 独立复核证据；Plan 归档为 done。 |
