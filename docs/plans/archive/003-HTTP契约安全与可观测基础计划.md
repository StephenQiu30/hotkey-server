...
canonical_path: docs/plans/archive/003-HTTP契约安全与可观测基础计划.md
status: archived
...
| 创建 | test/architecture/openapi_test.go | capabilities 正向路径和 `/healthz`、`/readyz`、`/metrics` 排除断言 |
| 修改 | test/tools/validate-architecture.sh | 仅允许 `internal/platform/http/result.go` 直接调用 Gin JSON 输出；业务 transport 禁止绕过统一 Handler |
...
- 创建：`docs/openapi/swagger.json`、`test/architecture/openapi_test.go`、`tools/tools.go`
...
- [ ] 运行 `go test ./internal/platform/http ./test/architecture -run 'Test(Capabilities|OpenAPI)' -count=1`，预期 capabilities handler 和 OpenAPI 文件缺失失败。
...
- 修改：`test/tools/validate-architecture.sh`、`Makefile`
- 测试：`test/architecture/openapi_test.go`、`internal/bootstrap/app_test.go`

- [ ] 写架构红灯：在临时业务 transport 文件中直接出现 `c.JSON(`、`c.AbortWithStatusJSON(` 或 `c.String(` 时，`test/tools/validate-architecture.sh` 必须失败；平台 `result.go` 是唯一允许写 JSON 的文件。
...
- [ ] 运行 `sh test/tools/validate-architecture.sh && make openapi-check && HOTKEY_TEST_DSN="$HOTKEY_TEST_DSN" make ci && make clean`，预期全通过且无 `hotkey` 二进制。
...
| 架构 | `sh test/tools/validate-architecture.sh` | 非平台业务 transport 无直接 Gin JSON 输出 |
