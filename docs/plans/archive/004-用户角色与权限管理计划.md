...
owner: HotKey Team
phase: P0
canonical_path: docs/plans/archive/004-用户角色与权限管理计划.md
...
- backend/test/_suite/internal/platform/http/
...
- [x] Slice-004-1：实现「用户列表、筛选和分页」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-004-* 回归。
- [x] Slice-004-2：实现「角色与状态调整」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-004-* 回归。
- [x] Slice-004-3：实现「删除与恢复」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-004-* 回归。
- [x] Slice-004-4：实现「权限不足状态和审计记录」，补齐实际涉及的领域、任务、API 与界面，并绑定 AC-004-* 回归。
