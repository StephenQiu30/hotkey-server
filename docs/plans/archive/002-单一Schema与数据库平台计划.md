...
canonical_path: docs/plans/archive/002-单一Schema与数据库平台计划.md
status: archived
...
inputs:
  - docs/prd/archive/002-单一Schema与数据库平台.md
...
| 创建 | test/postgresfixture/fixture.go | 每个集成测试单独创建并销毁数据库 |
| 创建 | test/tools/verify-database-runtime.sh、test/tools/generate-capacity-fixture.sh | 可丢弃库、DML、catalog 与容量计划门禁 |
| 修改 | internal/shared/repository/crud.go、internal/bootstrap/app_test.go | Repository 契约和运行角色回归覆盖 |
| 修改 | test/tools/validate-architecture.sh、test/tools/validate-repository.sh | 禁止 AutoMigrate 与业务表元数据门禁 |
...
| 基线 | go test ./test/architecture -run 'TestSchema|TestModelSchema' -count=1 | PLAN-001 的 Schema 和模型门禁保持通过 |
...
| 绿灯 | go test ./internal/platform/database ./internal/shared/... ./test/architecture -count=1 | 全部通过 |
