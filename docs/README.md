...
047 提供本地 Firecrawl、浏览器及候选平台服务的调研和接入设计；008 定义评论身份、父链、覆盖、增量和阅读。用户已确认 B站、小红书、抖音、微博评论均必需，四平台各自验收；网页闭环是基础，不替代任何平台验收。当前完成 S00/S01/S02：类型化受理、Kafka/Worker/Firecrawl、预算、原子页提交、持久任务结果与中断恢复，以及 URL 表单→任务→资料用户路径均已联通；S03 的隔离浏览器与仅允许测试域名的专用出口已有局部证据，真实平台闭环与产品 Acceptance 仍待执行。
