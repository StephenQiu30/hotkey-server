...
| 来源 | 033 | X 热点发现与持续热度观测 | 多源主链中的 X 增强，进行中 |
| 收敛 | 034 | X 热点核心链路收敛与遗留系统清理 | X-only 假设错误，已由 035 替代 |
| 收敛 | 035 | 多源 AI 热点监控首版收敛 | 已完成并通过验收 |
| 候选 | 036 | 评论会话采集与采集运行时边界 | 调研完成，待评审，不进入首版 |
...
flowchart LR
  ...
  F --> G["035：多源主链、单 ID 与首版边界"]
  G --> H["033：X 官方发现与持续指标增强"]
  H --> I["035：WebSocket、多维查询与过度设计清理"]
  I -. "035 验收后再评审" .-> J["036：按需评论会话增强（proposed）"]
```
