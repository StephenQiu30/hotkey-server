...
```text
docs/product/prd/NN-能力名称PRD.md
docs/plans/NN-能力名称实现计划.md
```

示例：

```text
docs/product/prd/02-关键词与用户偏好PRD.md
docs/plans/02-关键词与用户偏好实现计划.md
```
...
重建阶段允许清理历史遗留文档并重新编号，但必须满足：

1. 新编号以 Go 重建后的目标架构为基准，不沿用旧 FastAPI 计划编号。
2. `00` 保留给总览、迁移、仓库治理或文档重建任务。
3. `01-19` 覆盖 P0 开源核心闭环。
4. `20-39` 覆盖 P1 平台化能力。
...
| 2026-05-26 | StephenQiu30 | 1.0.1 | 增加 PRD 与 Plan 成对拆分、历史文档清理和重新编号规则 |
