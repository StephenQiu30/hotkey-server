...
3. PostgreSQL schema、pgvector、Redis 连接和本地启动路径可验证。
4. 可配置平台关键词和用户关键词偏好。
5. 可从至少一组国外事实源和一组传播源采集内容。
6. 可把相似内容聚合为事件簇，并展示相似度、匹配方式和合并理由。
7. 可展开热点事件，查看事实证据、传播证据和来源可信标签。
8. 可生成昨日平台日报和用户关注日报。
9. OpenAPI 可导出，Web 和小程序可基于契约生成客户端。
10. 任务失败、AI 失败、Redis 降级、pgvector 降级均有可追踪状态。
