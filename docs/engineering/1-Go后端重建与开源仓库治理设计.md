...
7. PR 提交粒度必须与 Epic 或 milestone 对齐；单个 task issue 只通过 feature commit、验收文档和 issue/Linear 评论跟踪，不单独创建 PR。
8. Epic PR 描述必须列出覆盖的 task issue、验证命令、OpenAPI 影响、未完成延迟项和里程碑关闭条件。
9. 里程碑推进过程中的临时记录、过程清单和一次性排查材料不进入 `docs/`。
...
| 2026-05-26 | StephenQiu30 | 1.1.1 | 明确 PR 粒度按 Epic 或 milestone 汇总，不按单个 task 提交 |
