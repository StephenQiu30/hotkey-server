...
3. fetch checkpoint 只在页面解析完成、每个 SourceItem 已幂等写入带版本化 `captured_item` 的 `collection_run_items`、每个 immutable target 已在 `collection_run_target_items` 对账且运行项计数一致后推进；PLAN-007 以该持久化捕获继续 Content/证据化，不重复请求来源。
