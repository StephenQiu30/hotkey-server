...
更新：2026-09-27，按当前工作树与 Plan 001、002、033 的隔离库证据重新核对。Design/PRD 001—007承担总纲与M1—M6 Epic；逐Issue计划从001连续到057，编号是身份，不是执行顺序。001、002、033 已在各自记录的范围内技术完成；其余计划中的新增表、接口、文件、测试仍是实施合同，未因列入索引就视为已创建、运行或验收。
...
3. 按具体CHK先保存失败/基线，再实现、运行适用门禁和本卡要求的局部真实验证。带真实条件的Plan不能仅因单测通过标completed；通常本卡全部CHK及真实条件通过后才标completed，即使跨卡父级产品AC仍待观测。若用户明确将本卡完成范围限定为技术交付，须在Plan、BACKLOG和Acceptance分别记录未通过或未提供证据的CHK子项，不能将该状态用于关闭父级AC；后续只消费已证实技术产物。
...
001—030为既有修订卡，031—052为新增/拆分卡，053—057接续审计缺口。001、002、033 的技术交付已 completed；001 的真实浏览器交互证据及父级 AC 仍待后续核对；026、046—050、053—054、057为blocked，其余planned；账号、渠道、真实样本和长窗另按各卡放行。表内Design编号对应同号PRD（001→PRD001，002→PRD002，依此类推），因而每行同时给出Design/PRD映射；现行文件路径由链接确定。
