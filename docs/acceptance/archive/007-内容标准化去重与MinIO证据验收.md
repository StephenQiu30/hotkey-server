...
在完整 legacy Schema 的可恢复副本上，执行 custom `pg_dump`、`pg_restore --list`、`docs/operations/002-内容标准化去重与MinIO证据升级.md` 的整个 transaction、当前 release `hotkey db verify`、手册中的精确外键清理与 `pg_restore`；最后切换到创建 backup 的 legacy release 再运行其 `hotkey db verify`。真实演练结果为：
