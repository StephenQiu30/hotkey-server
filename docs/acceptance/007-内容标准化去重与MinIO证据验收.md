...
canonical_path: docs/acceptance/007-内容标准化去重与MinIO证据验收.md
status: accepted
version: v1.0
...
- PostgreSQL、Redis 与 MinIO 均为本地可丢弃 fixture：`HOTKEY_TEST_DSN='postgres:///hotkey_plan007_test?sslmode=disable'`、`HOTKEY_TEST_REDIS_URL='redis://127.0.0.1:6379/15'`，以及 `scripts/plan007-minio-fixture.sh` 提供的 `127.0.0.1:19007` / bucket `hotkey-plan007`。fixture 在本验收结束后已执行 `down`；没有连接生产库、真实来源或真实凭据。
...
同一 PostgreSQL/Redis/MinIO fixture 下，`go test -race -tags=integration ./internal/modules/ingestion/... ./internal/modules/source/... ./internal/platform/database/... ./tests/architecture -count=1` 通过。验收结束后已执行 `sh scripts/plan007-minio-fixture.sh down`、`git diff --check` 与 `git status --short`；没有 fixture 容器、构建二进制或未提交实现改动。
