...
当前已实现 PostgreSQL job 唯一键、稳定 ID/version 载荷、到期判断和调度键，并支持在调用方事务中原子入队；Worker 已提供 SKIP LOCKED claim、完成、重试和达到上限隔离（提交 `1819b61`）。River 表幂等、事务回滚与 Worker integration test 已通过。

尚未完成：六类业务 Job、Worker Fx 生命周期、Cron 扫描、取消 API、P0 RSS/HN 端到端和恢复故障注入；保持 `pending`。
