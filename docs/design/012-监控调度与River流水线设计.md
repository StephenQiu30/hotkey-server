...
共享 `collection_run` 只执行一次来源请求；每个 `collection_run_target` 的 `source_checkpoint` 只在以下条件全部满足时推进：

1. 当前 shared run 页的原始证据已按来源策略保存或明确跳过。
2. 该 target 的每个 SourceItem 已写入 Content、标记确定性跳过或记录隔离失败。
3. 该 target 的下游任务已于同事务提交。
4. `collection_run_items` 的数量和分类与来源页面一致，且该 target 的处理结果完整。
