...
调研日期：2026-09-08。代码基线：`26b93d8a` 及调研时工作区。用户明确要求国内外平台同等重要，先通过免费接口和可维护网页采集验证需求。本文件记录设计提案，未安装第三方采集项目、登录平台、采集社交用户数据、改变运行服务或验证真实账号权限。来源公开文档证明的是能力存在，实际可用性仍需应用、账号、配额和具体用途验证。
...
| 能力 | 当前代码事实 | 本次需要补齐 |
|---|---|---|
| 来源类型 | `source/domain/connection.go` 有 RSS、HN、X、Bing Grounding、Bilibili、Weibo、Google Agent Search 七类 | 操作级能力及账号实测状态；不能按枚举数量计平台覆盖 |
| 关键词发现 | X recent search、微博 CLI 检索存在；B站适配器读取授权账号稿件，RSS 是固定 Feed，HN 是增量或榜单 | 真正的平台关键词查询、时间窗和排序；本地过滤单独标记 |
| 即时检索 | `source/application/instant_search.go` 拉取后本地匹配，当前只做一轮有界预览 | 持久搜索另走证据链；展示远端范围、页数、过滤后数量，不暗示全站召回 |
| 评论 | `SourceItem` 有 `ParentExternalID`，HN 可返回 comment | 评论不能只作为普通帖子处理；需要可查询父子关系、根帖、回复分页和评论证据 |
| 标准化 | `ingestion/application/normalizer.go` 将 HN comment 归为 post，标准化结果没有传递 parent | 保留来源 kind 与会话语义，避免评论被误计为独立事件信源 |
| 持久化 | `contents`、`source_observations` 没有可直接查询的 root/parent 关系；原始 captured item 可保留 parent JSON | 独立设计对象、版本、证据与会话关系，不沿用旧表正确性假设 |
| 可靠执行 | 已有 SourceConnection、Rights、Credential、Target、Checkpoint、River、MinIO | 评论父级独立 Checkpoint、操作级预算、网页解析漂移检测 |
| 分析与交付 | 已有相关性、事件、Heat、Python Agent、通知、知识与报告模块 | 评论目标情感/立场、观点主题、跨语言事件归并及覆盖质量 |

表内路径除特别注明外相对 `backend/internal/modules/`；数据库事实以 [`backend/db/schema.sql`](../../backend/db/schema.sql) 为准。以上仅为静态库存与反例，不证明这些模块可用、正确或应当保留。尤其不能把 Bing/Google 的检索摘要当成平台原始帖子或评论证据。

既有 001–005 文档把评论和更多来源放在 P1，并限制来源形态为 API/RSS/授权 Feed。006 是这次新增范围，需在实施时同步相关范围条款；本次不改写既有验收结果，也不将其状态自动升级。
...
同一 `server/` Python 包划分以下业务模块，各模块拥有自己的表和应用服务，不强制每个模块生成多层空目录。
...
本次仅交付设计三件套。后续在根 `server/` 建立新的 Python 工程，原 backend/agent 作为隔离的历史实现保留至替换验收；不让新服务调用旧业务 API、读取 River 队列或依赖旧权限判断。先打通新库的身份→监控→真实来源→评论→事件→引用，逐切片验证后再切换。

Alembic 是新库演进链，单一发布任务执行迁移，API/Worker 启动仅校验版本；autogenerate 产物必须人工审查，不能视为完整变更意图。CI 比较空库重放与上版新库升级，检查索引、约束和关键数据；迁移发布后不可改写，禁止自动破坏性 down。[Alembic 自动生成限制](https://alembic.sqlalchemy.org/en/latest/autogenerate.html)

旧库作为待核查导入源，不登记成新库迁移基线。导入先生成只读清单：来源、原 ID、hash、权限依据、原始证据与关系可信度；用户/来源权利重新确认，旧会话不导入。能够证实的内容经新标准化器进入带 provenance 的历史导入批次，无法证实的隔离，缺父关系保持未知，不从正文推断。分别报告读取/接受/拒绝/未知数量与原因，允许完全不导入。历史导入不发实时告警。

切换按 workspace/connection + operation 保证单一采集所有者：冻结旧写入、停止旧调度、等待或终止有界执行，记录最后水位与预算，再启动新任务；不让两套采集争抢账号额度。新工作台、新 OpenAPI、身份会话和新库共同切换，旧登录状态不作为新权限凭证。

失败时先停止新入口与副作用、保留新事实和删除账本，修复或恢复最近验证的新版本。回到旧系统并不等于可安全继续相同功能，需明确能力降级且不得恢复已删除正文。备份恢复、导入清单与删除屏障必须演练；本次用户授权重新设计不等于删除现有业务数据。

替换验收要求新业务完整运行时 Go 服务、River 与旧 Agent 全部停用仍可完成核心闭环。移除旧目录、旧镜像/CI/Schema 与过时治理条款是验收后的单独收尾步骤；不长期保留双业务事实。
