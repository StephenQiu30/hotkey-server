...
2026-09-15 规划入口：[007 热点事件与评论知识库设计](007-热点事件与评论知识库设计.md)。007 承接下一轮产品、数据与技术规划；本文件保留当前基础设计和历史研究/实现边界，不因新规划自动改变验收状态。
...
API 的有界数据库操作使用同步路由和独立同步 Session；Celery 采用 prefork 的同步任务入口，每任务使用自己的同步 Session，连接池在子进程初始化，不跨 fork 继承连接。HTTPX/Playwright 异步执行留在受监督执行器的自身事件循环；不把同步数据库调用放入 `async def` 路由，不把 `async def` Celery task 当作原生协程任务，也不在并发请求间共享 Session。[SQLAlchemy Session 并发边界](https://docs.sqlalchemy.org/en/20/orm/session_basics.html)、[FastAPI并发与async/await](https://fastapi.tiangolo.com/async/)
...
候选接口：来源能力查询；Monitor 预览/发布中的发现与评论策略；事件评论列表/树的有界展开；帖子会话采集；运行覆盖；分析结果和证据回看。新 API 统一以 `/api` 发布，由 FastAPI/Pydantic 生成 OpenAPI，再生成 TypeScript Client；不对旧 Go DTO 做默认兼容。具体方法、权限和 DTO 写入 Plan；尚未发布到 OpenAPI。
...
本轮补齐单owner初始化/会话撤销/CSRF、监控草稿与乐观锁、任务查询/有界分页/幂等诊断和取消。未连接来源前不发布采集任务；不伪造数据或兼容旧接口。当轮工作台以React/TypeScript/Vite重建，通过唯一docs/openapi/openapi.json生成类型并以openapi-fetch调用；007 S00 已将其替换为 `@umijs/openapi` 生成完整端点客户端和共享 Axios 传输层。
...
先实现 `src/sources/` 的类型契约、查询预览和 Bluesky 有界只读适配器，CLI 位于 `src/cli/sources.py`。不把网络请求放进 API 路由或诊断 Worker。预览接口只返回编译结果，不发起采集；来源能力仍保持 not_connected，直到持久化任务链验收。

首片查询为单条原生文本、固定 UTC 时间窗口和 latest 排序，保留分页 cursor；不承诺布尔语义、精确召回或全量分页。每次 CLI 操作只请求一页或一棵有界线程，固定 public.api.bsky.app，不接受任意 URL，不跟随跳转，不自动重试。响应解码后最多 2 MiB，线程深度最多 3、输出节点最多 200，HTTP 各阶段超时 10 秒并检查 20 秒流读取截止；这不是浏览器进程级硬截止。

输出保留 post/comment/reply、root/parent URI、未知回复数、不可见节点、返回字节/hash 和覆盖限制。线程深度/节点截断均标 partial；无 cursor 只表示此接口没有下一页，不表示平台数据完整。HTTP 401/403/429、超时、无效 JSON/字段、超限与合法空结果分别处理。真实来源正文不纳入 Git；实测记录只存操作、状态、数量和边界。

上游依据：[searchPosts Lexicon](https://github.com/bluesky-social/atproto/blob/main/lexicons/app/bsky/feed/searchPosts.json)、[getPostThread Lexicon](https://github.com/bluesky-social/atproto/blob/main/lexicons/app/bsky/feed/getPostThread.json)。国内来源仍需单独验证准入和解析，不以海外线程成功替代国内关键词与评论验收。
