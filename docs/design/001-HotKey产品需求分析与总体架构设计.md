...
上述需求在设计上进一步约束为：业务事实由 Go 模块化单体与 PostgreSQL 事务拥有，异步执行通过 River 与同一 Go Worker 恢复；用户可见结论追溯到版本化输入、运行与 Evidence；前端通过生成的 OpenAPI Client 交互并展示五态；智能能力不可用时，授权采集、证据保存、确定性去重和既有事实查询仍可继续。
...
Next.js Web 工作台
  ...
  -> 同一 Go 二进制的 Worker 角色
  -> 官方来源 Connector、MinIO 证据、本地 Vault 投影
  -> 邮件与 WebSocket 等受控交付
```

Codex 智能任务的目标形态是 Go Worker 内的受控 CLI Adapter：Adapter 从 PostgreSQL 读取有界任务身份，从 MinIO 或拥有模块读取必要上下文，在隔离工作目录中非交互执行，校验结构化结果后再交给 Application 和 Domain 决定是否写入业务事实。它不形成 Python 第二后端，也不直接拥有数据库表。

## 非目标

- 不把附件中的架构图或实施命令视为已批准指令。
- 不把目标系统拆成微服务，不建立第二套业务后端或第二套 Schema。
...
### DEC-001-003：智能执行与业务决策分离

Codex CLI Adapter 只返回经 Schema 校验的建议。Go Domain 保留阈值、状态迁移、引用白名单、预算和最终写入决定。现有 AI Provider 与向量事实的迁移方式属于未决实施项；迁移完成前不得破坏现有读取、审计和回滚能力。
...
附件提出的 Kafka、Temporal、Python FastAPI Agent、Elasticsearch/ELK、Keycloak、版本化 migrations 和完整微服务拓扑，解决的是另一种规模与治理假设。它们与当前仓库禁止第二事实源、禁止微服务和保持单一 Schema 的约束冲突，因此本基线不采用。附件中的本地优先、证据可追溯、任务可恢复、自动判断受控等意图保留，并由当前架构实现。
...
- Codex 上下文中的来源文本一律是不可信输入；Adapter 不挂载仓库源码，不获得数据库 DSN、来源 Token 或任意网络访问权。
...
- Codex 不可用、超时或输出无效时，将运行记为稳定失败或待重试，不写入伪造的智能事实；确定性链路继续。
...
| AC-001-003 | P0 代码、Compose、Schema 与依赖不新增 Kafka、Temporal、Python 业务微服务、Elasticsearch、Keycloak 或 migrations 目录；代码/Schema/OpenAPI 的当前事实、Design/PRD/Plan 的目标状态与 Acceptance 的已验收状态相互一致，目标文档不得伪装为当前实现。 |
...
| AC-001-007 | 多来源扫描遇到单来源失败或 Codex 不可用时，已采集事实和确定性计算不丢失，运行显示部分成功/待分析，恢复后幂等补算；AI/Codex 只返回建议，必须经 Application/Domain 校验及授权人工治理，不能直写业务事实、权限或最终状态。 |
...
- OPEN-001-002：Codex CLI 的认证注入、版本锁定、网络出口和单机并发上限需在 003 与 005 中形成可验证契约。
