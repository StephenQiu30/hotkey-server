...
2026-09-15。本文将本轮 GitHub、Firecrawl、Context7 研究和鱼皮源码分析落实到当前 HotKey。状态为设计提案，尚未实施。
...
| DEC-007-004 前端与交互 | React + TypeScript + Vite + openapi-fetch；沿用现有样式，按需加入图表组件 | 无 SSR/SEO 需求，不切 Next.js；首版轮询任务状态，暂不引入 Socket.io |
...
| HTTP客户端契约 | openapi-fetch 0.17.0、openapi-typescript 7.13.0 | 已固定；唯一OpenAPI和生成文件位置保持不变 |
...
前端依赖方向 `app → features → shared → api.generated`；shared不导入features，功能模块不互相深层导入实现，跨功能组合由app完成。共享API客户端只负责会话/CSRF/通用错误，Monitor/Job等类型引用放功能模块，避免client成为业务总入口。只有多处使用的UI才提升shared/ui；不用index.ts大范围重导出形成隐式依赖。
...
- OpenAPI生成位置和frontend/src/api.generated.ts不因前端分层移动；构建产物dist、node_modules、缓存、临时Playwright报告继续排除Git。
...
当前架构检查的 `PACKAGES` 是固定名单，未对所有未来目录自动扫描；前端也尚无对应依赖边界检查。S00先补未知目录检测、全源码扫描和负向依赖测试，新增模块必须同时登记其边界。上述目录图是目标规范，自动化检查未实现前不宣称已保证所有新文件的位置。
