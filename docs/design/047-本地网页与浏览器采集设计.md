...
- `sources/adapters/browser_runtime.py` 只接收已经由可信调用方解析的 Playwright `StorageState` 对象，不接受任意状态文件路径；每次调用仍创建独立、非持久 BrowserContext。创建 context 与交互共用默认/最大 45 秒协作式截止；退出、超时或取消时，按 Playwright 顺序分别以 `wait_for(5)` 请求关闭 context 与连接，即使前者超时也尝试后者。[Python 3.12 asyncio 文档](https://docs.python.org/3.12/library/asyncio-task.html#asyncio.wait_for)说明 `wait_for` 会等待被取消协程结束，故 5 秒不是进程级硬上限；WS 建连、Playwright 管理器退出与关闭耗时也不在交互计时内，不能代替业务任务总截止。状态文件的离线存取测试亦不等于已实现人工登录、认证失效或真实平台出站。
