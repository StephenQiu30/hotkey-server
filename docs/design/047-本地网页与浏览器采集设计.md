...
- `sources/adapters/browser_runtime.py` 只接收已经由可信调用方解析的 Playwright `StorageState` 对象，不接受任意状态文件路径；每次调用仍创建独立、非持久 BrowserContext，交互阶段默认 45 秒硬截止且不允许配置超过 45 秒，退出、超时或协作式取消时关闭 context。该计时不覆盖连接建立与清理耗时，不能代替业务任务整体 45 秒门禁；状态文件的离线存取测试也不等于已实现人工登录、认证失效或真实平台出站。
