...
日期：2026-09-22。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。当前为设计建议，未向 HotKey 加入依赖、启动服务或实现采集；Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。
...
本次调研始于 server HEAD `a33d2a2c`，提交前复核到 `42574ae1`。004 配置/启停 API/UI 等仍有其他任务的未提交改动，不属于本专项发布；已提交能力与工作树先行输出分开记录。

| 现有位置 | 已有能力 | 本专项要补的缺口 |
|---|---|---|
| `sources/contracts.py` | search / author_posts / comments / replies、作品/评论、分页与停止原因 | 没有按 URL 获取网页正文的契约；网页不能伪装成社交作品 |
...
| `connections/` | 已提交连接版本、证据版本屏障，probe 与 persisted_read 分立；配置/启停 API/UI 为其他任务未提交输出 | 来源目录目前只列 X/抖音，并遍历相同能力；匿名连接、浏览器状态及真实外采时版本/凭据解析仍需接入 |
...
004 已提交证据 `connection_version` 与停用/换版拒写；并行工作树还准备了服务端凭据引用及配置/启停 API/UI。047 实施前核对后者对应提交与验证，复用已交付能力，不重新实现。匿名连接、浏览器状态文件解析、真实外采前的执行门禁及页级事务仍需对应切片接入；本轮没有代替 004 运行验收。
