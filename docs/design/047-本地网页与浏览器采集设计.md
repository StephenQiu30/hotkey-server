...
accepted_scope: "S00；S01；S02 网页业务闭环；S03-T00/T01 隔离浏览器基础、专用出口拓扑与控制面/生命周期局部切片"
---
...
网络调用使用硬截止时间；首期每任务 1 URL，单次 Firecrawl 获取上限 20 秒，Browser 管理器启动、建连、context 与交互共用至多 45 秒截止。默认 Job lease 为 65 秒、Kafka max.poll.interval 为 120 秒。Settings 启动校验只计入已启用执行器，要求最长启用操作加清理预算（Browser context/连接/管理器各 5 秒）及 5 秒完成余量不超过 lease，且 lease 后至少保留 5 秒 poll 余量；CLI 与 BrowserRuntime 共用受限执行超时。该校验只保证配置组合可容纳已知运行预算，不提供进程级硬杀，也不等于整任务持久截止/崩溃恢复。多页社交任务在 S04 明确逐页续租、可恢复有界单元和整任务硬上限，超过单条上限保存 partial 并由后续任务续采；不在 Kafka 消息内无限滚动。
