...
accepted_scope: "S00；S01 网页契约与适配器基础"
---

# 本地网页与浏览器采集设计

日期：2026-09-22。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。v1.3 接受 S00 以及 S01 的网页契约、目标约束和默认关闭适配器基础；Firecrawl 既有服务恢复、上游日志/出口修复、显式探针、真实普通页/动态页和业务闭环仍未验收。Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。
...
本次调研始于 server HEAD `a33d2a2c`。S01 开工基线为 HotKey `c9889ea9`（004 S03 内部技术闭环已提交）与独立 Firecrawl `7666c1f`（v2.11.162）；HotKey 开工工作树干净。Firecrawl 工作树已有不属于本专项的构建/网络改动与脚本文件，两个既有容器均停止且 3002 无监听，因此本片未覆盖、未提交该仓库，也未启动第二套依赖服务。

| 现有位置 | 已有能力 | 本专项要补的缺口 |
|---|---|---|
| `sources/contracts.py` | search / author_posts / comments / replies、作品/评论、分页与停止原因；S01 已增加独立 page_content 文档契约 | 网页不能进入社交 `SourcePage`；持久网页对象仍待 S02 |
| `sources/adapters/x_twscrape.py` | 固定 SDK、有界单会话请求与映射 | 受控验证不等于 X 已接入；没有浏览器采集器 |
| `worker/app.py` | 租约、取消、失败、完成的装配 | `_registered_job_handlers()` 返回空字典，没有实际业务执行者 |
| `jobs/execution.py`、`content/services.py` | 检查点与作品持久化各自有事务 | `persist_post()` 和 `save_checkpoint()` 分别提交，不能直接拼接成带 fencing 的页级原子提交 |
| `connections/` | 已提交连接版本、证据版本屏障、服务端凭据引用及配置/启停 API/UI；S01 已将目录能力改为逐来源显式登记 | 当前只列 X/抖音的四类社交能力；web 来源、匿名连接、浏览器状态及真实外采时版本/凭据解析仍需接入 |
| `content/`、`evidence/` | 正文版本、观察、去重、生命周期、来源政策 | 入库服务当前只接受 post；原始网页对象写入和孤儿对象回收仍需真实实现 |
| 本机独立 Firecrawl 仓库 | 固定 `v2.11.162`，API + 渲染器配置，复用宿主依赖 | 本次 docker 查询两个容器均停止；上一轮健康请求无法连接 3002。当前浏览器服务源码还会记录 URL/headers |

004 已提交证据 `connection_version`、停用/换版拒写、固定服务端凭据映射及配置/启停 API/UI。047 复用这些能力，不重新实现。匿名连接、浏览器状态文件解析、真实外采前的执行门禁及页级事务仍需对应切片接入；本轮没有代替 004 运行验收。

S01 已实现的边界是：`page_content` 使用独立 `WebPageRequest` / `SourceDocument` / `WebPageResult` / `DocumentAdapter`；目标只允许调用方明确列出的精确公共 HTTP(S) 域名，拒绝 IP 字面量和非默认端口，移除 fragment 并保留路径/query 语义；Firecrawl 只调用固定本地 `/v2/scrape`，发送 `maxAge=0`、`storeInCache=false`、20 秒/100000 字/2 MiB 上限，不接受任意 headers、Cookie、脚本、代理或服务地址。结果分别校验服务 HTTP、`success`、目标状态、最终地址、正文及登录/验证码特征；只报告一次 collector call，无法观测的目标请求数保持 unknown。能力默认关闭，尚无 CLI 探针或业务调用者。
...
| Scrapling 组件候选 | Python Selector 解析、AsyncDynamicSession 浏览器动作 | 底层仍为 Playwright；先在 S03 与直接调用比较，不作为已选依赖，不接管业务调度/checkpoint |
...
Scrapling 的高 star 使其进入 S03 优先比较名单，不构成生产采用结论。0.4.15 受控实验中，动作异常/等待超时仍可返回 200，自适应定位可能返回另一条评论；如采用，来源适配器必须记录动作结果并校验稳定 ID、作品、父链和页终点，将异常映射到失败/部分状态，不能依赖 SDK 返回码。普通 HTTP 继续用既定 HTTPX；如果需要其 curl_cffi 请求引擎，先登记明确的来源 SDK 例外。默认不启用自适应身份替换、开发响应缓存或完整 Spider 调度；特征 SQLite 和日志亦须落实隔离、脱敏及删除。研究细节和发布证据见 [047 Research 第 6 节](../research/047-本地网页与浏览器采集调研.md#6-scrapling按-star-筛选再核对实际能力)。
...
partial 沿用既有 `partially_succeeded` 状态；当前 `complete()` 在未取消时直接写 succeeded，必须由处理器返回类型化完成结果，再由统一包装持久化正确的完成状态与阶段结果。重复游标/无新增 ID 触发有界停止，失败不推进完整水位。客户端超时不代表 Firecrawl 后端已经停止，超时后的资源占用与有限回收须单独测试。
...
本机 Firecrawl `apps/playwright-service-ts/api.ts` 当前打印请求 URL 和 headers，是已核实的运行缺口。恢复公开 URL 探针前去掉原始 URL/query/header/body 日志，并验证所用镜像包含修改；不把“没有传 Cookie”当作已满足敏感 URL 脱敏。Firecrawl 内部缓存、队列记录和日志也属于保留范围。
...
|---|---|---|
| 当前宿主机开发 | 配置指向 `http://127.0.0.1:3002`，恢复后验证 | S03 再启用受管运行时，worker 用固定版本客户端连接 |
...
上述默认连接使用 Playwright 原生协议。Scrapling 0.4.15 的 `cdp_url` 使用 `connect_over_cdp()`，不能直接接收该 Server 的 WS 地址。若 S03 选用它的浏览器组件，须先比较“保留原生 Playwright、仅复用 Selector”和“受控 CDP 运行时”的隔离、取消和维护代价，再同步本节、PROJECT 与配置契约后实施；不为了候选同时常驻两套浏览器。当前临时本机 Chrome 实验没有证明远程连接兼容或 Linux 容器能力。
...
**DEC-047-008：工具调用量与目标站请求量分开计量。** 一次 Firecrawl API 调用会触发数量不透明的目标请求，不能记作“目标站只请求了一次”。037 增加明确的 `collector_call` 用量/预算维度；原 `network_request` 只报告可观察的请求并带统计范围。上游未知数保持 unknown；浏览器可见请求在请求发出前计量，重定向/失败也消耗预算。硬上限由时间、并发、字节和出口共同限制；目标请求总量未被观测时，不宣称全量网络计量已验收。
...
- 本地：独立仓库 `/Users/stephenqiu/Desktop/StephenQiu/Firecrawl` 内的 `LOCAL-DEPLOYMENT.md`、`docker-compose.local.yaml`、`apps/playwright-service-ts/api.ts` 源码/配置核对，以及 `docker ps -a --filter name=firecrawl`。服务源码不属于 HotKey；实施修改在其独立仓库单独审阅。
