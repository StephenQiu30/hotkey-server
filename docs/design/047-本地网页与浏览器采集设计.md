...
accepted_scope: "S00；S01；S02 网页连接、域名绑定、计量枚举、持久化与页提交基础"
---

# 本地网页与浏览器采集设计

日期：2026-09-22。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。v1.7 接受 S00/S01 及 S02 的网页连接、域名绑定、计量枚举、持久化与页提交基础：`web` 无凭据连接、不可变允许域名、执行版本/目标门禁、collector_call 枚举、webpage 内容模型及带租约 fencing 的原子页提交已落地；实际预算预留/结算、网页业务任务和 UI 尚未验收。Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。
...
| `jobs/execution.py`、`content/collection.py`、`content/services.py` | 已提供事务内租约/检查点与网页持久化入口；页用例锁定未取消的当前 epoch、连接版本、政策/保留版本，将正文、观察、发现、生命周期、持久读取证据和 checkpoint 原子提交 | 尚未由真实 Worker 处理器调用，collector_call 的实际预算预留/结算仍待接入 |
...
**DEC-047-008：工具调用量与目标站请求量分开计量。** 一次 Firecrawl API 调用会触发数量不透明的目标请求，不能记作“目标站只请求了一次”。037 的 `collector_call` 用量/预算维度已同步 Pydantic、ORM 与唯一 DDL；原 `network_request` 只报告可观察的请求并带统计范围。上游未知数保持 unknown；浏览器可见请求在请求发出前计量，重定向/失败也消耗预算。硬上限由时间、并发、字节和出口共同限制；处理器尚未接入实际预留/结算，因此不宣称任务计量已验收。
