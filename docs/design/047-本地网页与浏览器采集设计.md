...
accepted_scope: "S00；S01；S02-T01 与 S02-T02 后端业务闭环"
---

# 本地网页与浏览器采集设计

日期：2026-09-23。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。v1.9 接受 S00/S01 及 S02-T01/T02 后端闭环：`webpage.collect` 由服务端固定连接版本与目标，经 Job/Outbox、Kafka、Worker、Firecrawl、预算结算和原子页提交形成持久资料与任务结果；重放、换版、限流、未知历史 kind、调用结算前崩溃和页提交后中断均有持久恢复边界。网页 URL 表单、任务结果跳转和浏览器交互仍属 T03，产品 AC 仍未建立。Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。
