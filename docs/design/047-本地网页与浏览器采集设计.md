...
accepted_scope: "S00；S01；S02 网页连接、域名绑定、collector_call 预算调用与页级持久化基础"
---

# 本地网页与浏览器采集设计

日期：2026-09-22。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。v1.8 接受 S00/S01 及 S02 的网页连接、域名绑定、collector_call 预算调用与页级持久化基础：`web` 无凭据连接、不可变允许域名、执行版本/目标门禁、真实预算预留/实际结算、webpage 内容模型及带租约 fencing 的原子页提交已落地；网页业务任务和 UI 尚未验收。Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。
...
| `jobs/execution.py`、`jobs/services.py`、`content/collection.py`、`content/services.py` | 已提供事务内租约/请求/预算/用量/检查点入口；网络编排在短事务内锁定 owner/operation/epoch、连接版本与目标，原子预留 collector_call 并开始尝试，调用后按实际次数结算；页用例将正文、观察、发现、生命周期、持久读取证据和 checkpoint 原子提交 | 两段用例尚未由真实 Worker 处理器串联；历史未注册 kind、调用后崩溃恢复与持久任务结果仍待 T02 |
| `connections/` | 已提交连接版本、证据版本屏障、服务端凭据引用及配置/启停 API/UI；`web` 只声明 page_content，以 `auth_kind=none`、空 secret_ref 和规范化 allowed_hosts 建立不可变版本；事务内门禁锁定当前版本并校验目标 | 业务任务尚未固定/传递连接版本；browser state 及页级最终提交门禁仍需接入 |
| `content/`、`evidence/` | 正文版本、观察、去重、生命周期、来源政策；已增加 webpage URL 身份、最终跳转观察字段、机器提取来源与事务内政策/保留锁 | 尚无网页任务受理、真实处理器或产品 UI；原始 HTML/截图仍默认不保存 |
| 本机独立 Firecrawl 仓库 | 固定 `v2.11.162`，API + Playwright 复用宿主依赖；中央日志脱敏、无原始请求日志、目标 IP 校验与固定解析已提交，既有两个容器运行且 readiness=200 | 当前只证明宿主 CLI 技术探针；HotKey Docker worker 到该控制面及 S03 独立浏览器仍待验证 |

004 已提交证据 `connection_version`、停用/换版拒写、固定服务端凭据映射及配置/启停 API/UI。047 将认证版本扩展为 `none`/`server_credential`：公开网页不保存或伪造秘密，并把规范化精确域名写入不可变版本；执行门禁在调用者事务内锁定 owner/连接/当前版本并校验目标。浏览器状态文件解析、任务固定该版本及页级最终提交仍需后续接入。

S01 已实现的边界是：`page_content` 使用独立 `WebPageRequest` / `SourceDocument` / `WebPageResult` / `DocumentAdapter`；目标只允许调用方明确列出的精确公共 HTTP(S) 域名，拒绝 IP 字面量和非默认端口，移除 fragment 并保留路径/query 语义；Firecrawl 只调用固定本地 `/v2/scrape`，发送 `maxAge=0`、`storeInCache=false`、20 秒/100000 字/2 MiB 上限，不接受任意 headers、Cookie、脚本、代理或服务地址。结果分别校验服务 HTTP、`success`、目标状态、最终地址、正文及登录/验证码特征；只报告一次 collector call，无法观测的目标请求数保持 unknown。能力由 0600 的本地环境显式启用；CLI 探针只输出稳定状态与计数且不持久化，尚无业务调用者。
...
**DEC-047-008：工具调用量与目标站请求量分开计量。** 一次 Firecrawl API 调用会触发数量不透明的目标请求，不能记作“目标站只请求了一次”。037 的 `collector_call` 用量/预算维度已同步 Pydantic、ORM 与唯一 DDL；原 `network_request` 只报告可观察的请求并带统计范围。上游未知数保持 unknown；浏览器可见请求在请求发出前计量，重定向/失败也消耗预算。S02-T01 已实现实际调用路径：当前租约、owner/operation、连接版本和目标与预算预留、用量开始、请求计数在同一短事务内完成，网络事务外执行，随后按 `collector_call_count` 原子结算预算和用量结果；预算/零成本组件政策缺失时闭锁。该证据尚未串入 Kafka/Worker，不能据此宣称业务任务计量或产品 AC 已验收。
