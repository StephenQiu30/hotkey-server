...
accepted_scope: "S00；S01；S02 匿名网页连接版本子片"
---

# 本地网页与浏览器采集设计

日期：2026-09-22。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。v1.5 接受 S00/S01 及 S02 的匿名网页连接版本子片：网页技术探针与既有 Firecrawl 已验证，`web` 无凭据连接边界已落库；网页业务任务、持久资料和 UI 尚未验收。Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。
...
| `connections/` | 已提交连接版本、证据版本屏障、服务端凭据引用及配置/启停 API/UI；`web` 只声明 page_content，并以 `auth_kind=none`、空 secret_ref 建立不可变匿名版本 | 网页执行尚未绑定连接版本/域名范围；browser state 及真实外采时版本解析仍需接入 |
...
004 已提交证据 `connection_version`、停用/换版拒写、固定服务端凭据映射及配置/启停 API/UI。047 复用这些能力，并将认证版本扩展为 `none`/`server_credential` 条件约束：公开网页版本不保存或伪造秘密，X/抖音仍要求服务端凭据。浏览器状态文件解析、真实外采前的版本/域名执行门禁及页级事务仍需对应切片接入；本轮没有代替 004 运行验收。
