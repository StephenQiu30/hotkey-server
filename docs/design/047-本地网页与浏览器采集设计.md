...
accepted_scope: "S00；S01；S02 网页连接、域名绑定与计量子片"
---

# 本地网页与浏览器采集设计

日期：2026-09-22。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。v1.6 接受 S00/S01 及 S02 的网页连接、域名绑定与计量子片：`web` 无凭据连接、不可变允许域名、执行版本/目标门禁和 collector_call 已落地；网页业务任务、持久资料和 UI 尚未验收。Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。
