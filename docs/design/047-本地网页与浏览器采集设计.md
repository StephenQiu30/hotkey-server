...
accepted_scope: "S00；S01；S02 网页业务闭环；S03-T00/T01 隔离浏览器基础、专用出口拓扑、控制面/生命周期局部切片及 Worker 单任务进程监督决策"
---

# 本地网页与浏览器采集设计

日期：2026-09-24。本文回应本地部署 Firecrawl / Playwright、减少来源接口依赖的需求。v1.10 接受 S00/S01 及完整 S02 网页业务闭环：`webpage.collect` 由服务端固定连接版本与目标，经 Job/Outbox、Kafka、Worker、Firecrawl、预算结算和原子页提交形成持久资料与任务结果；现有 `/content` 可提交 URL，任务页可取消、手动重试、呈现部分状态并打开持久资料。重放、换版、限流、未知历史 kind、调用结算前崩溃和页提交后中断均有持久恢复边界。产品 Acceptance 仍未建立，四平台评论、隔离浏览器与恢复验收继续按 S03—S05 推进。Scrapling 临时环境受控研究见 047 Research，不计入业务验收。承接 004—010、034、036—039、042 的既有范围；044/045 可在以后复用。本专项不增加产品 PRD、P0 分母，不替代 X 和国内来源的独立验收。2026-09-24 对现有 Firecrawl 的 `example.com` 受控探针收到 `SCRAPE_RETRY_LIMIT`/`document_antibot`；HotKey 现将该结构化错误映射为 `access_denied`，但未取得页面正文，不能据此宣称当前采集成功。
...
| `worker/execution.py` | 尚无任务进程监督；父进程同步执行来源 handler | S03 需要单任务总截止、取消/超时强制回收及未知崩溃未确认重放 |
...
| 本机独立 Firecrawl 仓库 | 固定 `v2.11.162`，API + Playwright 复用宿主依赖；中央日志脱敏、出站校验及此前 HotKey Kafka/Worker live 单页链曾通过 | 2026-09-24 现有实例对 `example.com` 返回 `SCRAPE_RETRY_LIMIT`/`document_antibot`，当前探针无正文；适配器已将该结构化拒绝分类为 `access_denied`，但不代表采集成功 |
...
- `WebPageResult`：文档或稳定失败原因、目标页面 HTTP 状态、可验证的用量及其统计范围。HTTP 200、Firecrawl success、目标页面成功和有效正文必须分别检查；登录/验证码页不能算资料。Firecrawl 5xx 中结构化 `SCRAPE_RETRY_LIMIT` 且错误类型为 `document_antibot` 映射到既有 `access_denied`；其他 5xx 映射到 `upstream_error`，不向领域结果暴露上游原始错误文本。
...
网络调用使用硬截止时间；首期每任务 1 URL，单次 Firecrawl 获取上限 20 秒，Browser 管理器启动、建连、context 与交互共用至多 45 秒截止。Worker 父进程另外监督整条任务的单调时钟总截止；Browser 最大运行路径预算为 45 秒交互 + 三段各 5 秒清理 + 5 秒 handler 准备 + 最多 3 秒子进程准备 + 2 秒终止宽限 + 5 秒数据库终结，默认 Job lease 为 75 秒、Kafka `max.poll.interval.ms` 为 120 秒。启动校验只计入已启用执行器的操作/清理上限，并校验 spawn、handler、终止和终结余量及 lease 后 5 秒 poll 余量。配置校验不代替父进程对真实 handler 的硬截止；多页社交任务在 S04 明确逐页续租、可恢复有界单元和整任务硬上限，超过单条上限保存 partial 并由后续任务续采，不在 Kafka 消息内无限滚动。

### 4.2 单任务硬截止、取消与崩溃恢复

`worker/messaging.py` 的父进程独占 Kafka 消费与同步 offset；每条已取得 lease 的业务任务由 `worker/execution.py` 使用 Python 标准库 `multiprocessing.get_context("spawn")` 启动独立子进程，子进程自行创建数据库 Engine/Session 和 handler 资源。不得传递父进程 Consumer、Session、Engine、套接字或浏览器上下文，不增加进程池、依赖或服务。子进程只接收无秘密的任务/owner/operation/config 版本与 lease，并通过一次性 Pipe 报告 ready、成功、类型化失败或未分类崩溃；原始异常正文和敏感数据不跨管道、不写日志。

父进程以一个 monotonic 总截止监督启动和执行，同时每 0.5 秒检查 Worker 停止与 PostgreSQL 中当前 lease 的取消标志。取消或已知总截止时，先终止子进程，最多等待 2 秒，仍存活则 kill。子进程确认退出后，父进程在同一数据库事务内保守结清本网页任务未完成的 Firecrawl usage attempt，并完成 cancelled 或稳定的 `job_execution_timeout` 失败/inbox；事务成功后才提交当前 Kafka offset。已发出但结果未知的 collector call 保守计费。超时允许人工重试，不默认无界自动重试。

若子进程以未分类异常退出或 IPC 结果缺失，父进程不写成功/失败终态且不提交 offset；关闭 Consumer，等待数据库当前 lease（含 checkpoint 续租）到期后重新创建 Consumer 并重放原 offset。已提交的 page checkpoint 继续避免重复上游采集。Worker 收到正常停止信号时限时终止活动子进程并保留当前 offset 未提交；父进程硬崩溃下由既有容器运行时回收 Worker 容器内进程，重启后沿用 lease/offset 恢复。Browser 是独立远程服务，客户端子进程退出后必须释放对应 renderer/context；本次不改变其 Compose 拓扑。
...
- S03-T02 的离线状态存储由 `connections/adapters/local_secrets.py` 管理：操作者提供已存在的绝对 0700 根目录，根及派生目录均拒绝符号链接；以 owner/连接 UUID 与正整数版本生成不可变 `browser-state:` 引用，不接受调用方文件名。保存使用同目录临时 0600 文件与不可覆盖的原子发布；读取校验引用、权限、文件类型、大小与 JSON 形状，失败不回显正文。此存储组件不自行创建连接或判定版本当前有效，后续 CLI/业务调用仍必须经 004 的连接行锁与版本屏障。
- `sources/adapters/browser_runtime.py` 只接收已经由可信调用方解析的 Playwright `StorageState` 对象，不接受任意状态文件路径；每次调用仍创建独立、非持久 BrowserContext。Playwright 管理器启动、WS 建连、创建 context 与交互共用默认/最大 45 秒协作式截止，[Playwright 连接](https://playwright.dev/python/docs/api/class-browsertype#browser-type-connect)本身另有更短超时；退出、超时或取消时，按 Playwright 顺序分别以 `wait_for(5)` 请求关闭 context、连接与管理器，即使前一步超时也尝试后一步。若业务操作和清理同时失败，保留业务操作异常，并仅附加清理异常类型，避免清理错误覆盖业务结果或泄漏异常文本；若业务操作成功但清理失败，则向调用方传播清理错误。管理器启动中取消也请求停止本地驱动。[Python 3.12 asyncio 文档](https://docs.python.org/3.12/library/asyncio-task.html#asyncio.wait_for)说明 `wait_for` 会等待被取消协程结束，故每步 5 秒不是进程级硬上限；三个关闭步骤均不计入前述 45 秒，也不能代替业务任务总截止。状态文件的离线存取测试亦不等于已实现人工登录、认证失效或真实平台出站。
- `source_connection_versions.auth_kind` 增加 `browser_state`，且其 `secret_ref` 必须等于该行 owner/连接/版本派生的 `browser-state:` 引用；唯一 DDL、ORM 和内部枚举同步。执行前由 `connections.services` 在调用方事务内锁定 owner 的连接，检查 active、当前版本及当前版本认证失效证据，再通过受控存储读取状态。旧版本、停用、失效、缺文件均不得进入 BrowserContext；外部请求后的最终提交仍由既有租约/连接版本屏障复核。此类型先为后续平台接入准备，不在未实现适配器时增加来源目录或对用户开放连接写接口。
- Playwright 的 `storage_state()` 不自动持久化 `sessionStorage`；S04 必须按实际平台登录样本确认 Cookie/localStorage/IndexedDB 是否足够，依赖未覆盖的会话状态时停止并要求重新登录，不把测试文件存在视为认证有效。[官方认证说明](https://playwright.dev/python/docs/auth#session-storage)
- 人工登录状态采集优先复用 Playwright 官方 `codegen --save-storage` 的独立浏览器会话；它在浏览器关闭后导出 Cookie/localStorage/IndexedDB，不创建 HotKey 一次性登录脚本，也不读取个人日常浏览器 profile。输出文件属于高敏凭据，后续维护 CLI 必须校验受控目录/权限、原子换版与当前连接，且只能由操作者在本地触发；目前尚未新增浏览器认证来源，不能把 `codegen` 导出文件直接视为已激活连接。[官方代码生成器说明](https://playwright.dev/python/docs/codegen#preserve-authenticated-state)
- 维护 CLI 首片只针对已存在的 `browser_state` 连接：操作者提供捕获文件路径，文件与父目录需分别为 0600/0700、常规文件/目录且无符号链接；CLI 不打印状态正文或路径。`connections.services` 按 owner/连接行锁与 expected_version 保存新状态文件并追加不可变版本；停用也追加版本，但新版本不复制旧状态文件，且拒绝新执行，重新启用必须再导入人工登录状态。同版本重复停用不再增版。保存文件先于数据库提交，若事务结果不明可能留下未引用文件；仅在行锁下确认当前版本未前进且目标文件内容完全相同时允许同一捕获重试，不覆盖不同状态。无现有 browser_state 连接时 CLI 明确拒绝，不预先增加假来源或从 API 建立秘密连接。首版平台连接初始化仍由 S04 在适配器和准入明确后接入。
...
**DEC-047-007 的 S03 固定实现：**只采用 Playwright Python `1.63.0` 的原生 `BrowserType.connect()` WebSocket；浏览器服务使用匹配的 `mcr.microsoft.com/playwright:v1.63.0-noble` 镜像，并在构建时固定 Server 包版本。Scrapling 0.4.15 的 `cdp_url` 走 `connect_over_cdp()`，不兼容该原生 WS；同样本实验中动作异常/等待超时仍返回 200、自适应误认评论 ID，默认日志输出完整 URL/query。故本期不引入 Scrapling/Selector、CDP 服务、SQLite 特征库或响应缓存，普通 HTTPX 与业务 Kafka/PostgreSQL checkpoint 不变。T01 已随实际 runtime/CLI 调用方锁定 Python 包与镜像并验证容器内连接；macOS 机制实验仍不代替真实平台和生产 Linux 主机验收。[同样本记录](../research/047-本地网页与浏览器采集调研.md#66-s03-t00-同样本复核与选型2026-09-23)

S03-T01 已交付的最小安全出口为默认断网：browser 只连接与 Worker 共享的内部 Compose 网络，Worker 保留原业务网络；browser 无数据库/对象存储网络、端口映射、宿主挂载或 Docker socket。基于固定 Playwright 镜像构建的 Node `launchServer` 显式启用 `chromiumSandbox`；非 root 用户、只读根文件系统、tmpfs、官方 seccomp 与最小 `SYS_CHROOT` 通过 Linux-arm64 Docker Desktop 实测。最初直接使用官方 `run-server` 时，实际 Chromium 带 `--no-sandbox`；切换入口后检查主进程参数已无该标志。Worker 同镜像的一次性容器通过内部 WS 运行 `set_content` 无网络探针，browser 的公共 IP 连接返回 `ENETUNREACH`，`postgres` 无法解析，宿主数据库端点无法连接，容器挂载数为 0、宿主端口映射为空。该证据不证明公开目标可访问，也不代表生产 Linux 主机验收。S04 打开真实平台出站前仍必须另证 DNS/重定向/子资源的受控出口与请求预算；不通过时不开放平台目标，不把 Worker 的业务网络通过 `expose_network="*"` 透传给页面。

S03 专用出口选择（2026-09-23）：HotKey 自有一个固定版本 Squid 实例，不借用 Video 的代理配置或网络。browser 保留与 Worker 的内部 WS 网络，另接只含 browser/代理的 `internal` 网络；只有代理再接公网桥接网络。Playwright Server 启动 Chromium 时统一指定该代理，并撤销 Chromium 对 loopback/link-local 的隐式代理绕过；不向用户脚本暴露代理配置。代理不发布宿主端口、不记录 URL/query，首片只放行 `example.com` 的 80/443 受控探针，拒绝 IP 字面量、私网/保留地址、其他域名及代理入口的非必要方法；HTTPS CONNECT 内部方法不由代理识别。Docker Desktop 的公开 DNS 合成地址 `198.18/15` 仅对该明确域名放行，不把本机现象当作 DNS 重绑定的完整证明。这个网络切片只证明受限出口可用，不放行平台目标；后续 S04 必须按来源准入分别登记域名/子资源、重定向与实际解析及请求预算，验证后才可扩展代理允许清单。单靠浏览器代理配置并不能替代容器网络隔离或业务侧目标校验。

S03 控制面路径防护：`HOTKEY_BROWSER_WS_URL` 由本机忽略版本控制的环境文件提供，格式为 `ws://browser:3000/ws/` 加 48 个随机十六进制字符；Compose 将同一个值注入 browser 与调用方。browser 从 URL 中取路径传给 Playwright `launchServer(wsPath)`，不配置、根路径、弱令牌或不同形状均启动失败；调用方启用 browser 时同样拒绝空值/弱路径。该 URL 相当于控制面凭据，不进日志、异常正文、Git 或公开响应；根 Compose 仍不发布 3000 端口。[Playwright 对 wsPath 的安全提示](https://playwright.dev/docs/api/class-browsertype#browser-type-launch-server)。这仅收窄已知 WS 路径，不能替代内部网络隔离、任务权限和后续平台请求计量。

组件依据：[Playwright `launchServer` 代理参数](https://playwright.dev/docs/api/class-browsertype#browser-type-launch-server)、[Chromium 隐式 loopback 绕过](https://chromium.googlesource.com/chromium/src/+/main/net/docs/proxy.md#implicit-bypass-rules)、[Squid 有序访问规则](https://www.squid-cache.org/Doc/config/http_access/)。
...
- 本地：独立仓库 `/Users/stephenqiu/Desktop/StephenQiu/Firecrawl` 的 `6d9fb16`、`LOCAL-DEPLOYMENT.md`、当时的本地 Compose 覆盖配置、API/Playwright 源码、容器 readiness/日志及真实普通页、动态页、DNS/私网/重绑定探针。当前本机入口已统一为 `docker-compose.yml`，服务源码不属于 HotKey，提交与运行证据在独立仓库单独审阅。
...
| 2026-09-23 | v1.12 | 接受 S03-T01 固定 browser target、非 root Chromium sandbox、内部 WS 与默认断网的 Worker 远程探针；会话/平台出站及产品 AC 待后续。 |
| 2026-09-23 | v1.13 | 明确 S03-T02 的离线 browser state 目录/引用/权限/原子发布和显式对象注入；受控 live 状态往返通过，连接换版/登录与真实平台仍待后续。 |
| 2026-09-23 | v1.14 | `browser_state` 版本行受 DDL 精确引用约束，执行前行锁复用当前版本/停用/认证失效屏障；空库真实 PostgreSQL 用例通过，来源注册与人工登录仍待后续。 |
| 2026-09-23 | v1.15 | S03-T02 交互阶段上限与超时/取消关闭通过远程 browser 实测；一次性客户端硬退出后渲染进程回到基线，业务恢复和人工登录仍待后续。 |
| 2026-09-23 | v1.16 | 明确本地维护 CLI 只操作已有 browser_state 连接；捕获文件权限、换版事务及停用再启用经受控数据库验证，真实平台初始化与业务任务仍待后续。 |
| 2026-09-23 | v1.17 | S03-T02 让 context 创建与交互共用截止，关闭步骤各有协作式等待预算；本机 live 重连通过，业务任务总截止与进程级强制回收仍未证明。 |
| 2026-09-23 | v1.18 | 将远程 WS 建连纳入 context 与交互的同一截止；管理器进入/退出、关闭及业务任务总截止仍需独立约束。 |
| 2026-09-23 | v1.19 | 接受 HotKey 专用 Squid 出口拓扑与 `example.com` 受控允许清单；平台域名、WS 路径及业务请求计量仍待 S04。 |
| 2026-09-23 | v1.20 | browser 与调用方共用私有不可猜测 WS 路径；根路径拒绝及现有容器受控 live 回归通过，平台请求计量和 G4-002 仍待后续。 |
| 2026-09-23 | v1.21 | 管理器启动纳入浏览器执行截止，退出增加协作式限时清理；真实业务总截止与 G4-002 仍待后续。 |
| 2026-09-24 | v1.22 | 固定业务异常与资源清理异常并发时的优先级及脱敏规则；不改变真实平台准入、业务总截止或 G4-002 范围。 |
| 2026-09-24 | v1.23 | Browser/Firecrawl 执行预算、资源清理、Job lease 与 Kafka poll 窗口加入启动期交叉校验；Worker 整任务硬截止和崩溃恢复仍未实现。 |
| 2026-09-24 | v1.24 | 冻结 Worker 父进程/任务子进程职责、spawn 隔离、整任务硬截止、取消资源结清与未知退出未提交 offset 等待 lease 重放；实现和 G4-002 证据仍待验证。 |
| 2026-09-24 | v1.25 | 将 Firecrawl `SCRAPE_RETRY_LIMIT`/`document_antibot` 稳定映射为 `access_denied`，其他 5xx 保持 `upstream_error` 且不暴露原始文本；当前探针仍无可用正文，采集/Acceptance 不计通过。 |
