...
- `sources/adapters/browser_runtime.py` 只接收已经由可信调用方解析的 Playwright `StorageState` 对象，不接受任意状态文件路径；每次调用仍创建独立、非持久 BrowserContext 并在退出/取消时关闭。状态文件的离线存取测试不等于已实现人工登录、认证失效或真实平台出站。
