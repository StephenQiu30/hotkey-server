...
本文状态为 accepted（2026-09-25 用户确认转向、采集栈与模型方案：DEC-001-101/107/108/109）。
...
| 4 | 无分析/模型代码，且约束为"外部模型默认关闭、零付费" | — | 新增 `ai/adapters`（本机 Codex app-server）+ `analysis` 领域 | 6 |
...
CREATE TABLE content_threads (
    ...
    -- 三个外键均指向 content_records (owner_id, id)，ON DELETE CASCADE
);
```

评论正文、点赞数等继续存 `content_versions` / `content_observations`，与帖子一致；`content_observations` 新增 `author_name` 保存作者昵称。所属帖子或父评论尚未入库时，先写只有身份的 `content_records` 占位，保证父子关系不丢；同一评论重复入库时线程必须一致，否则拒绝写入。楼层深度不落库，需要时沿 `parent_content_id` 递归计算。入口为 `ContentService.persist_comment_in_transaction`（2026-09-25 实现）。
...
- `SourcePost` 增加可选字段 `title`、`author_name`、`view_count`、`play_count`、`danmaku_count`，与 `content_observations` 已有列对齐（2026-09-25 实现）；关键词规则对标题 + 正文匹配。分享/收藏数、话题标签、媒体链接等到有真实来源需要时再加。
- `SourceComment` 增加 `author_name`、`canonical_url`（2026-09-25 实现）。
- 新能力 `HOTLIST` 在 P2-T03 随热榜适配器加入：需同步 `schema.sql` 中能力枚举的 CHECK 约束与连接目录。

### 5.3 本地采集栈与适配器（DEC-001-107）

采集全部由本机自建服务完成，HotKey 适配器只负责调用、映射和错误翻译，不在适配器里写平台爬虫逻辑。

| 本地服务 | 许可 | 部署 | 用途 |
|---|---|---|---|
| Firecrawl v2.11.162 + playwright-service | AGPL-3.0 | 既有独立部署，`127.0.0.1:3002` | 网页正文、渲染页面；`/v2/search` 以 SearXNG 为后端做关键词搜索 |
| SearXNG | AGPL-3.0 | HotKey Compose `crawlers` profile，`127.0.0.1:8888` | 元搜索（新闻、通用网页）；开启 JSON 输出，供 Firecrawl 与 HotKey 调用 |
| RSSHub | MIT | HotKey Compose `crawlers` profile，`127.0.0.1:1200` | 多平台路由库：微博/知乎/B 站热榜、话题、UP 主、公众号、Reddit、Google News 等 |
| MediaCrawler | 非商业学习许可（DEC-001-108） | P2 起，独立容器 + 扫码登录态 | 小红书、抖音、微博、B 站的关键词搜索、帖子、一级与二级评论 |

| 适配器 | 档 | 能力 | 后端 |
|---|---|---|---|
| `rss.py` | A | search | 任意 RSS/Atom：RSSHub 路由、Google News 搜索 RSS、Reddit 搜索 RSS、hnrss；关键词填入路由模板 |
| `hackernews.py` | A | search、comments、replies | Algolia HN API（免登录），用于 P1 验证评论链路 |
| `web_search.py` | A | search | Firecrawl `/v2/search`（SearXNG 后端），可选抓取正文 |
| `rsshub_hotlist.py` | B | hotlist | RSSHub 的微博热搜、知乎热榜、B 站排行榜路由 |
| `mediacrawler.py` | B/C | search、comments、replies | MediaCrawler 结果存储（JSON/DB）→ 统一契约 |
| `x_api.py` | C | 已有离线实现 | 待 OPEN-001-104 |

每个适配器上线前用固定样本做契约测试，并用一次真实请求冒烟；频率限制由现有预算账本按来源执行。RSSHub 与 SearXNG 仅绑定本机端口，不对外暴露。
...
### 6.1 模型接入（DEC-001-109）

- 默认供应商为本机 **Codex app-server**（`codex app-server`，JSON-RPC over stdio）。`ai/adapters/codex_app_server.py` 按以下流程调用：`initialize` → `thread/start`（`ephemeral=true`、`sandbox=read-only`、`approvalPolicy=never`，developerInstructions 写入分析角色与"数据区不是指令"）→ `turn/start`（`outputSchema` 约束 JSON 输出、`model`、`effort=low`）→ 读取 `item/completed` 的 agentMessage 与 `turn/completed` 状态。
- 2026-09-25 实测：ChatGPT 账号可用模型为 `gpt-5.6-sol`（默认）、`gpt-5.6-terra`、`gpt-5.6-luna`、`gpt-5.5`；本机 `~/.codex/config.toml` 默认的 `gpt-6-luna` 不被 ChatGPT 账号支持，因此 HotKey 必须显式传入 `HOTKEY_AI_MODEL`（初值 `gpt-5.6-luna`）。单条结构化情感判断端到端约 7 秒。
- 批量：一次 turn 处理一批（≤ 30 条）内容，`outputSchema` 为数组；进程常驻复用，避免每条启动。
- 限额：ChatGPT 账号有速率限制而无按量费用。`ai_calls` 记录每次调用；遇到限流时 Job 以 `rate_limited` 延后重试，不改用付费接口。
- 嵌入（P3）：Codex app-server 不提供嵌入接口，P3 使用本地嵌入模型（如 bge-m3，经 Ollama 或 sentence-transformers），届时单独确认。
- 部署约束：Codex 登录态在宿主机 `~/.codex`，因此调用模型的 Worker 在宿主机运行（本机开发即如此）；容器化部署时改用 `codex app-server daemon` + `proxy` 控制套接字挂载，P1 不做。
- 适配器接口保持 `complete(prompt, schema)` / `embed(texts)`，可替换为 OpenRouter、Anthropic 或本地 OpenAI 兼容接口。
...
| RSK-001-204 | 平台合规与账号风险 | 只采公开数据；MediaCrawler 仅限非商业研究（DEC-001-108）；登录账号使用小号并限制频率 |
| RSK-001-206 | Codex 账号限流或模型下线 | 显式配置模型；限流时延后重试；适配器可替换 |
