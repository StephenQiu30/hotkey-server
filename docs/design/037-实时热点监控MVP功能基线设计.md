...
version: v1.0
owner: HotKey Team
canonical_path: docs/design/037-实时热点监控MVP功能基线设计.md
prd: docs/prd/037-实时热点监控MVP功能基线.md
plan: docs/plans/037-实时热点监控MVP功能基线计划.md
supersedes: docs/design/035-多源AI热点监控首版收敛设计.md
...
HotKey 当前 MVP 收敛为一个清晰的关键词热点监控工具：用户配置监控词，系统定时或按需从多个来源发现新内容，完成去重、时效过滤和 AI 内容判断后形成热点卡片，并把新结果实时送到页面和邮件；用户也可以执行一次性多源搜索，或通过 Agent Skill 生成热点报告。
...
本次复核覆盖参考仓库的 README、需求说明、定时任务、来源服务、AI 服务、数据库模型、REST 接口、Socket 客户端、完整 Web 页面和 Agent Skill，并检查了公开截图。关键事实如下。
...
| Agent Skill | 8+ 来源搜索和报告 | 独立 Python Skill 可一次性搜索 8 源并生成 Markdown | 作为独立 P0 入口，但复用 HotKey API，不建设第二套采集器 |
| 设置/日报 | 需求文档中出现 | KV API 与日报函数没有 Web 调用方 | 不纳入对标基线 |

参考证据：六项宣传能力见 [README 35–68 行](https://github.com/liyupi/yupi-hot-monitor/blob/cd48b0885bfa8ae9c8043cf78ef6cfd045530bdb/README.md#L35-L68)；Web 六源调用见 [hotspotChecker 76–108 行](https://github.com/liyupi/yupi-hot-monitor/blob/cd48b0885bfa8ae9c8043cf78ef6cfd045530bdb/server/src/jobs/hotspotChecker.ts#L76-L108)；30 分钟调度见 [index 69–78 行](https://github.com/liyupi/yupi-hot-monitor/blob/cd48b0885bfa8ae9c8043cf78ef6cfd045530bdb/server/src/index.ts#L69-L78)；Agent Skill 的八源边界见 [SKILL 41–54 行](https://github.com/liyupi/yupi-hot-monitor/blob/cd48b0885bfa8ae9c8043cf78ef6cfd045530bdb/skills/hot-monitor/SKILL.md#L41-L54)。
...
- 在支持 Skill 的 AI 工具中，用户可以说“搜索 Claude 最新热点”或“生成今日 AI 热点报告”。
- Skill 调用 HotKey 的搜索/热点 API，不直接复制参考项目的 HTML 抓取脚本。
- 报告包含扫描时间、成功/失败来源、重要性分组、摘要、相关性、热度和原文链接。
- Skill 是 Web 能力的另一入口，不维护独立数据库、来源凭据或热点判断逻辑。
...
8. 响应式桌面/移动工作台。
9. 复用同一 API 的 Agent Skill 搜索与 Markdown 报告。
...
| 参考能力 | Web 自动监控 | Skill 即时搜索 | HotKey 合规实现 |
|---|---:|---:|---|
...
| 搜狗类中文网页发现 | 必须具备同类能力 | 必须具备同类能力 | 仅授权搜狗 Feed/API；未获授权时使用明确标名的合法中文搜索源，不冒充搜狗 |
...
| Google 搜索 | 非参考 Web 主链要求 | 必须 | Google Agent Search 或授权搜索 API |
| DuckDuckGo | 非参考 Web 主链要求 | 必须 | 官方 Instant Answer 能力；UI 明确它不是完整网页搜索 |
| RSS/Atom | 可追加 | 可追加 | 标准 RSS/Atom，作为 HotKey 的额外合法来源 |

“六源 Web、八源 Skill”是参考提交的真实产品边界，不得再写成含混的“Web 自动监控 8+ 来源”。若某个来源未配置或未获得授权，产品只展示实际可用来源，发布说明不能把它计入已接通数量。
...
- Agent Skill 只调用 HotKey API，并能生成包含来源成功/失败状态的 Markdown 报告。
