...
010 的来源范围以 jobs 所属单一持久范围状态记录确认与缺口，Job checkpoint 只记录单次执行位置，content 继续持有实体、版本和观察；确认水位由同一作用域连续已证实的窗口推导，不按最大已见发布时间或 Job 完成状态推断。S01 已将范围表、ORM、服务与受控测试同批加入唯一 DDL；现有开发库未就地迁移，也没有真实多页处理器。首个真实来源接入前不设置通用重叠时长、虚构来源可用性或提前创建回补 API/UI。细则见 [010 Design](docs/design/010-增量更新与历史回补设计.md)。
