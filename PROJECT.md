...
按总体 Design → 需求/Plan → 失败验证 → 实现 → 回归/Acceptance 推进。研究、范围和设计可先开展；[046 前置计划](docs/plans/046-全局异常与响应契约前置计划.md) S03 已通过，[042](docs/plans/042-容量与部署可重复性计划.md) S00/S01 已交付底座，[027](docs/plans/027-数据正确性计划.md) S00 已冻结身份/观察前置，[031](docs/plans/031-可靠执行与幂等计划.md) S00—S02 已交付任务恢复语义，[036](docs/plans/036-数据访问与生命周期计划.md) S00—S02 已交付来源准入与生命周期屏障，[037](docs/plans/037-费用与资源约束计划.md) S00—S02 已交付免费组件准入、全尝试计量与分层预算预留，[038](docs/plans/038-可维护与可替换计划.md) S00/S01 已交付纯来源语义与适配器端口，[039](docs/plans/039-可观测与可运维计划.md) S00/S01 已交付任务关联与统计口径。下一步按 B02 顺序推进 028 S00/S01 的可追溯与可复现切片。001 B00 的外部来源条件继续登记，不阻塞内部领域切片；技术前置通过不等于产品验收。
