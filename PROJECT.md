...
010 的来源范围设计以 jobs 所属单一持久范围状态记录确认与缺口，Job checkpoint 只记录单次执行位置，content 继续持有实体、版本和观察；确认水位由同一作用域连续已证实的窗口推导，不按最大已见发布时间或 Job 完成状态推断。首个真实多页来源接入前可用受控分页样本实现内部契约，但不设置通用重叠时长、虚构来源可用性或提前创建回补 API/UI；范围状态表只在 S01 实际服务与测试同批进入唯一 DDL/ORM。细则见 [010 Design](docs/design/010-增量更新与历史回补设计.md)。
