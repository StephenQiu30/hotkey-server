...
更新日期：2026-09-24。本文固定仓库边界、技术栈、后端目录、API 契约和运行约束。
...
上述 P0/P1/P2 继承现有 PRD 草案，不因本轮文档修订变成已经批准的工期。X 必需；2026-09-23 用户将 X 自动采集改为官方 API，作为按量付费内容来源的唯一明确例外。开发者凭据和账期费用上限未确认前不发起真实请求；其他来源与基础分析仍按免费或自建约束执行。既有 MinIO、Python/Next.js/Flutter 方向不变。Web 首版与 App 独立验收，App 的未决平台不阻塞已确定的 Web 交付。

### 1.3 当前实现边界（2026-09-24 核对）

本轮从 server HEAD `12441116` 开始执行。已有后端应用工厂、健康/文档路由、Worker/CLI 入口、依赖锁、唯一根 Compose 及基础 CI；046 已补齐统一异常/响应契约、请求标识与脱敏、Web 传输分类、可控同源代理失败、运行时 OpenAPI 客户端生成和契约漂移 CI；042 S00/S01 已补真实 PostgreSQL/Redis/Kafka、空库初始化、资源上限和部署态运行门禁；034 S00/S01 已补单 owner、服务端会话、CSRF、注销及维护恢复，`schema.sql` 已登记身份和会话表；035 S00/S01 已补默认拒绝 owner 规则、受保护工作区 API、登录页及 `/events` 空工作台；031 S00—S02 已补持久任务、事务 outbox、手动 offset、inbox、租约/检查点恢复与有限调度追赶；036 S00—S02 已补来源准入、字段最小化、从严保留、即时读取屏障和在线清理；037 S00—S02 已补免费组件准入、全尝试计量、分层窗口、原子预留、幂等结算/释放与耗尽延期；038 S00/S01 已补纯来源能力请求、作品/评论、分页/水位、停止原因与结构化适配器端口；047 S00—S02 已补公开网页契约/目标约束、匿名连接、collector_call、固定 `webpage.collect` Worker、真实 Kafka/Firecrawl、原子持久结果及 URL 表单→任务→资料入口，S03-T03 已补共享 Worker 单任务进程监督与 Browser renderer 取消释放的技术验证；039 S00/S01 已补任务运行上下文、阶段尝试、互斥状态与三类尝试汇总；027 S00 已冻结内容身份与不可变观察边界；028 S00/S01 已补不可变输入清单、比较参考集、方法版本、稳定指纹、幂等冲突和当前生命周期屏障；029 S00/S01 已补计划到期事实、严格时区/顺序校验、分阶段整数微秒和来源时间异常分类；032 S00/S01 已补同快照数据库候选归档、证据对象引用核对和秘密隔离。隔离浏览器与平台会话、四平台真实评论处理器、凭据上下文隔离/换版旧写端到端证明、真实评分/模型记录、最后成功/延期/缺口/陈旧传播、维护审计、独立介质/真实恢复/回补清理、完整容量和产品 Acceptance 仍待交付。

046 的本地测试、生产构建、真实 Next/FastAPI 代理和桌面/窄屏浏览器验证记录在对应 Acceptance；它不证明数据库、消息、来源、部署或产品闭环。后续应扩展现有底座，不以旧的“空工程”描述重建第二套实现。

010 的来源范围以 jobs 所属单一持久范围状态记录确认与缺口，Job checkpoint 只记录单次执行位置，content 继续持有实体、版本和观察；确认水位由同一作用域连续已证实的窗口推导，不按最大已见发布时间或 Job 完成状态推断。S01 已将范围表、ORM、服务与受控测试同批加入唯一 DDL；S02 时间标记子片要求范围 Job 显式固定 `scan_kind`，作品发现投影按任务区分追新、刷新和历史回补，不用入库时间改写原始发布时间。现有开发库未就地迁移，也没有真实多页处理器、游标恢复或通知抑制。首个真实来源接入前不设置通用重叠时长、虚构来源可用性或提前创建回补 API/UI。细则见 [010 Design](docs/design/010-增量更新与历史回补设计.md)。
...
浏览器适配器把 Playwright 管理器启动、WS 建连、context 创建与交互纳入同一次默认/最大 45 秒协作式截止；context、连接和管理器按序各有 5 秒关闭等待预算，前一步失败仍尝试后一步。该浏览器级协作截止本身不是进程硬回收，也不是整条业务任务总截止；047 S03-T03 已在共享 Worker 层加入子进程级任务总截止、terminate→kill 回收、取消资源结清及未知退出后 lease 到期重放，并在现有 Browser 服务上验证 renderer/context 随取消释放。由于 Browser 业务处理器尚未接入该 Worker 路径，G4-002 的凭据上下文隔离和换版旧写端到端矩阵仍未通过；多页平台任务仍须由 S04 明确逐页续租、可恢复有界单元与整任务硬上限。

- 复用现有 MinIO；除 X 官方 API 这一例外外，内容来源采用免费或自建方案，只采集公开或获授权数据。X 为首版必需来源；2026-09-22 用户明确 B站、小红书、抖音、微博的评论与回复都需要，四个平台分别验收。已知作品 URL 评论路径可先于各平台全站搜索，不能以首个国内来源成功关闭四平台需求。契约见 [008 Design](docs/design/008-评论与回复采集设计.md)，候选与证据见 [047 Research](docs/research/047-本地网页与浏览器采集调研.md)。
- [002](docs/design/002-X免费采集与热点监控设计.md) 改走 X 官方 API；既有 `sources/adapters/x_twscrape.py` 仅是历史受控样本实现，不注册 Worker、不读取浏览器登录会话，也不作为收费 API 的替代路径。官方接入须先补开发者凭据、安全存储、账期硬上限和 037 付费例外门禁，再按能力验证。模型供应商、SDK、模型与设备仍按 [043](docs/design/043-模型服务接入与模型配置设计.md) 等专项处理，不能写成已接入。
