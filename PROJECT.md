...
更新日期：2026-09-21。本文固定仓库边界、技术栈、后端目录、API 契约和运行约束。
...
5. `worker/` 维护 Kafka 客户端和消费者生命周期，`jobs/` 维护任务状态机；入口 `python -m worker`。031 已实现 outbox、发布器、consumer group、inbox/offset、执行权、检查点、再均衡和有限调度恢复；039 S01 将不含运行上下文的 `hotkey.jobs.accepted.v1` 升级为 `hotkey.jobs.accepted.v2`，消息与任务绑定配置版本及来源能力；009 S01 已发布持久受理与 owner 隔离状态读取 API，009 S02 已实现 `begin_request` 持久请求计数、检查点进度、协作式取消、取消超时和 cancelled 消息确认。当前仍无具体业务处理器，009 S02 的同提交浏览器证据及 S03 的分类重试仍待执行；不能把受控处理器、排队记录或空 Worker 当作真实业务任务已执行。
...
按总体 Design → 需求/Plan → 失败验证 → 实现 → 回归/Acceptance 推进。研究、范围和设计可先开展；[046 前置计划](docs/plans/046-全局异常与响应契约前置计划.md) S03 已通过，[042](docs/plans/042-容量与部署可重复性计划.md) S00/S01 已交付底座，[009](docs/plans/009-采集任务控制计划.md) S00/S01 已交付任务持久受理与状态读取 API，S02 代码和非浏览器门禁已通过但同提交浏览器证据未关闭，[027](docs/plans/027-数据正确性计划.md) S00 已冻结身份/观察前置，[028](docs/plans/028-可追溯与可复现计划.md) S00/S01 已交付不可变输入清单与稳定指纹，[029](docs/plans/029-时效与数据新鲜度计划.md) S00/S01 已交付计划到期事实与分阶段时间链，[031](docs/plans/031-可靠执行与幂等计划.md) S00—S02 已交付任务恢复语义，[032](docs/plans/032-备份与恢复计划.md) S00/S01 已交付候选归档与依赖清单，[036](docs/plans/036-数据访问与生命周期计划.md) S00—S02 已交付来源准入与生命周期屏障，[037](docs/plans/037-费用与资源约束计划.md) S00—S02 已交付免费组件准入、全尝试计量与分层预算预留，[038](docs/plans/038-可维护与可替换计划.md) S00/S01 已交付纯来源语义与适配器端口，[039](docs/plans/039-可观测与可运维计划.md) S00/S01 已交付任务关联与统计口径。下一步先在不违反运行约束时补 009 S02 浏览器证据，再进入 S03。001 B00 的外部来源条件继续登记，不阻塞内部领域切片；技术前置通过不等于产品验收。
