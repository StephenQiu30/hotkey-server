...
func TestCompiledRecallProfileReaderServesExplicitSemanticUnavailabilityAndRetires(t *testing.T) {
	...
	monitorID, configID, revisionID := createPublishedCompiledProfileOwner(t, runtime, "unavailable")
	var profileID int64
...
) VALUES ($1,'published',$2,$2,$3,'intent-compiler-v1',$4,$5,$6,$7,'canonical-search-v1',
  'unavailable','semantic_model_unavailable') RETURNING id`, monitorID, configID, revisionID,
...
func TestCompiledRecallProfileRejectsReadySemanticWithoutExactEmbedding(t *testing.T) {
	runtime := openCompiledRecallProfileRuntime(t)
	defer func() { _ = runtime.Close() }()
	monitorID, configID, revisionID := createPublishedCompiledProfileOwner(t, runtime, "missing-embedding")
	var profileID int64
	if err := runtime.SQL.QueryRow(`
INSERT INTO monitor_compiled_profiles (
  monitor_id,purpose,config_version_id,monitor_version_id,intent_revision_id,
  ...
  semantic_algorithm_version,structured_algorithm_version,search_normalization_profile_version,
  semantic_state
) VALUES ($1,'published',$2,$2,$3,'intent-compiler-v1',$4,$5,$6,$7,'canonical-search-v1','ready')
RETURNING id`, monitorID, configID, revisionID,
		ingestionapplication.HybridRecallMatchingAlgorithmVersion, ingestionapplication.LexicalRecallAlgorithmVersion,
		ingestionapplication.SemanticRecallAlgorithmVersion, ingestionapplication.StructuredRecallAlgorithmVersion).Scan(&profileID); err != nil {
...
func createPublishedCompiledProfileOwner(t *testing.T, runtime *database.Runtime, suffix string) (int64, int64, int64) {
	t.Helper()
...
VALUES ($1,$2,$3,1,'track exact published event intent') RETURNING id`, draftID, monitorID, configID).Scan(&revisionID); err != nil {
	...
	return monitorID, configID, revisionID
}
