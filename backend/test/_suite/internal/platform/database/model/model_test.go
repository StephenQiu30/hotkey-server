...
func TestSpecsHaveUniqueTablesAndColumns(t *testing.T) {
	...
	wantColumns := map[string][]string{
		...
		"notification_delivery_claims":          {"user_notification_id", "channel", "delivery_target_key", "claim_token", "fencing_generation", "dispatch_key", "provider_supports_idempotency", "provider_supports_receipt_lookup", "dispatch_started_at", "claimed_at", "lease_until"},
		"notification_delivery_attempts":        {"id", "version", "user_notification_id", "channel", "delivery_target_key", "attempt_no", "status", "dispatch_key", "fencing_generation", "provider_supports_idempotency", "provider_supports_receipt_lookup", "provider_message_id", "response_code", "error_code", "attempted_at", "created_at"},
	...
	if got, want := len(seen), 154; got != want {
