...
func TestGORMCRUDUsesMappedVersionSoftDeleteAndCursor(t *testing.T) {
	...
	stale := *first
	stale.Version = 1
	if err := repo.Update(ctx, &stale); !errors.Is(err, sharedrepository.ErrConflict) {
	...
	page, err := repo.List(ctx, sharedrepository.PageQuery{Limit: 2, Sort: "id", FilterFingerprint: "all-users"})
	if err != nil {
	...
	next, err := repo.List(ctx, sharedrepository.PageQuery{Limit: 2, Sort: "id", FilterFingerprint: "all-users", Cursor: page.NextCursor})
	if err != nil {
	...
	if _, err := repo.List(ctx, sharedrepository.PageQuery{Limit: 2, Sort: "id", FilterFingerprint: "other-filter", Cursor: page.NextCursor}); !errors.Is(err, sharedrepository.ErrInvalidInput) {
		t.Fatalf("cross-filter cursor error = %v, want invalid input", err)
	}
	if _, err := repo.List(ctx, sharedrepository.PageQuery{Limit: 2, Sort: "id", Descending: true, FilterFingerprint: "all-users", Cursor: page.NextCursor}); !errors.Is(err, sharedrepository.ErrInvalidInput) {
	...
	if _, err := repo.GetByID(ctx, first.ID); !errors.Is(err, sharedrepository.ErrNotFound) {
	...
	if _, err := NewGORMHistory[model.User](runtime.GORM, "users"); !errors.Is(err, sharedrepository.ErrInvalidInput) {
		t.Fatalf("NewGORMHistory(users) error = %v, want invalid input", err)
	}
}

func TestMapErrorUsesStableCategories(t *testing.T) {
	if got := MapError(fmt.Errorf("wrapped: %w", gorm.ErrRecordNotFound)); !errors.Is(got, sharedrepository.ErrNotFound) {
		t.Errorf("MapError(wrapped gorm not found) = %v, want not found", got)
	}
	for _, test := range []struct {
		code string
		want error
	}{
		{code: "23505", want: sharedrepository.ErrConflict},
		{code: "40001", want: sharedrepository.ErrConflict},
		{code: "23503", want: sharedrepository.ErrConstraint},
		{code: "23514", want: sharedrepository.ErrConstraint},
		{code: "57014", want: sharedrepository.ErrUnavailable},
	} {
		if got := MapError(fmt.Errorf("wrapped: %w", &pgconn.PgError{Code: test.code})); !errors.Is(got, test.want) {
	...
	for _, contextErr := range []error{context.Canceled, context.DeadlineExceeded} {
		if got := MapError(contextErr); !errors.Is(got, sharedrepository.ErrUnavailable) {
