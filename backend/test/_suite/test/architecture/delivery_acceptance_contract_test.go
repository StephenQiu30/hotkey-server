...
func Test003And004PartialAcceptancesRecordHonestRemainingGates(t *testing.T) {
	repository := filepath.Clean(filepath.Join(repositoryRoot(t), ".."))
	tests := []struct {
		path           string
		docNo          string
		partialAC      string
		remainingCheck string
		indexLink      string
	}{
		{
			path:           "docs/acceptance/003-智能研判事件热度与人工治理验收.md",
			docNo:          "003",
			partialAC:      "AC-003-003",
			remainingCheck: "CHK-003-G5-001",
			indexLink:      "[部分验收](003-智能研判事件热度与人工治理验收.md)",
		},
		{
			path:           "docs/acceptance/004-通知报告知识投影与检索验收.md",
			docNo:          "004",
			partialAC:      "AC-004-008",
			remainingCheck: "CHK-004-G5-002",
			indexLink:      "[部分验收](004-通知报告知识投影与检索验收.md)",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.docNo, func(t *testing.T) {
			...
			if status := frontmatterStatus(t, filepath.Join(repository, test.path)); status != "failed" {
				t.Errorf("%s partial Acceptance status = %q, want failed", test.docNo, status)
			}
			...
			if !strings.Contains(acceptance, "| `"+test.partialAC+"` | `partial` |") || !strings.Contains(acceptance, "`"+test.remainingCheck+"` 保持未勾选") {
				t.Errorf("%s Acceptance does not preserve the exact remaining product gate", test.docNo)
			}
			if !strings.Contains(index, test.indexLink) || !strings.Contains(index, test.docNo+" |") {
				t.Errorf("Acceptance index does not expose the partial %s result", test.docNo)
			}
		})
	}
}
