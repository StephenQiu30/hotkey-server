...
CREATE TABLE IF NOT EXISTS source_request_usage_ledgers (
    ...
    rate_used bigint NOT NULL CHECK (rate_used > 0),
...
ALTER TABLE source_request_usage_ledgers ADD COLUMN IF NOT EXISTS rate_window_start timestamptz;
ALTER TABLE source_request_usage_ledgers ADD COLUMN IF NOT EXISTS rate_used bigint;
UPDATE source_request_usage_ledgers
SET rate_window_start = date_trunc('minute', updated_at), rate_used = used
WHERE rate_window_start IS NULL OR rate_used IS NULL;
ALTER TABLE source_request_usage_ledgers ALTER COLUMN rate_window_start SET NOT NULL;
ALTER TABLE source_request_usage_ledgers ALTER COLUMN rate_used SET NOT NULL;
ALTER TABLE source_request_usage_ledgers DROP CONSTRAINT IF EXISTS source_request_usage_ledgers_rate_used_check;
ALTER TABLE source_request_usage_ledgers ADD CONSTRAINT source_request_usage_ledgers_rate_used_check CHECK (rate_used > 0);
...
CREATE INDEX IF NOT EXISTS knowledge_change_proposals_lexical_fts_idx
    ON knowledge_change_proposals USING gin (
        to_tsvector('simple',COALESCE(proposed_frontmatter->>'title','') || ' ' || proposed_body)
    ) WHERE status='applied';
CREATE INDEX IF NOT EXISTS knowledge_change_proposals_lexical_trgm_idx
    ON knowledge_change_proposals USING gin (
        lower(COALESCE(proposed_frontmatter->>'title','') || ' ' || proposed_body) gin_trgm_ops
    ) WHERE status='applied';
...
CREATE TABLE IF NOT EXISTS retention_policies (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, version bigint NOT NULL DEFAULT 1,
    data_class varchar(64) NOT NULL UNIQUE CHECK (data_class IN ('captured_items','content_metric_snapshots','event_metric_snapshots','sessions','delivery_attempts','job_attempts','audit_logs')),
...
ALTER TABLE retention_policies DROP CONSTRAINT IF EXISTS retention_policies_data_class_check;
ALTER TABLE retention_policies ADD CONSTRAINT retention_policies_data_class_check CHECK (data_class IN ('captured_items','content_metric_snapshots','event_metric_snapshots','sessions','delivery_attempts','job_attempts','audit_logs'));
...
VALUES
    ...
    ('delivery_attempts', 90, 'delete', false, '受保护的投递尝试事实，普通保留任务不可删除'),
...
SET enabled=false,
    description=CASE data_class
        WHEN 'delivery_attempts' THEN '受保护的投递尝试事实，普通保留任务不可删除'
        ELSE '受保护的审计事实，普通保留任务不可删除'
    END
WHERE data_class IN ('delivery_attempts','audit_logs')
  AND (enabled OR description IS DISTINCT FROM CASE data_class
        WHEN 'delivery_attempts' THEN '受保护的投递尝试事实，普通保留任务不可删除'
        ELSE '受保护的审计事实，普通保留任务不可删除'
      END);
...
CREATE TABLE IF NOT EXISTS retention_runs (
    ...
    retention_policy_id bigint NOT NULL REFERENCES retention_policies(id) ON DELETE RESTRICT,
    ...
    candidate_count bigint NOT NULL CHECK (candidate_count BETWEEN 0 AND 1000),
    has_more boolean NOT NULL,
    candidate_hash char(64) NOT NULL CHECK (candidate_hash ~ '^[0-9a-f]{64}$'),
    status varchar(24) NOT NULL CHECK (status IN ('pending_approval','approved','completed','blocked')),
    ...
    executed_by_user_id bigint REFERENCES users(id) ON DELETE RESTRICT,
    executed_at timestamptz,
    affected bigint NOT NULL DEFAULT 0 CHECK (affected BETWEEN 0 AND 1000),
    ...
    CHECK (
        ...
        OR status='completed' AND approved_by_user_id IS NOT NULL AND approved_at IS NOT NULL AND executed_by_user_id IS NOT NULL AND executed_at IS NOT NULL AND affected=candidate_count AND failure_code IS NULL
        OR status='blocked' AND approved_by_user_id IS NOT NULL AND approved_at IS NOT NULL AND executed_by_user_id IS NOT NULL AND executed_at IS NOT NULL AND affected=0 AND failure_code IS NOT NULL
    )
);
CREATE INDEX IF NOT EXISTS retention_runs_status_idx ON retention_runs(status,created_at,id);

DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conrelid='retention_runs'::regclass
          AND conname='retention_runs_independent_approval'
    ) THEN
        ALTER TABLE retention_runs
        ADD CONSTRAINT retention_runs_independent_approval
        CHECK (approved_by_user_id IS NULL OR approved_by_user_id <> requested_by_user_id)
        NOT VALID;
    END IF;
END;
...
CREATE TABLE IF NOT EXISTS retention_run_items (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    retention_run_id bigint NOT NULL REFERENCES retention_runs(id) ON DELETE RESTRICT,
    ordinal integer NOT NULL CHECK (ordinal > 0),
    candidate_id bigint NOT NULL CHECK (candidate_id > 0),
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (retention_run_id,ordinal),
    UNIQUE (retention_run_id,candidate_id)
);

CREATE OR REPLACE FUNCTION enforce_retention_run_transition()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    IF TG_OP='DELETE' THEN
        RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='retention_runs_retained', MESSAGE='retention runs cannot be deleted';
    END IF;
    IF NEW.version IS DISTINCT FROM OLD.version
       ...
       OR NOT (OLD.status='pending_approval' AND NEW.status='approved'
            OR OLD.status='approved' AND NEW.status IN ('completed','blocked')) THEN
        RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='retention_run_transition', MESSAGE='invalid retention run transition';
    END IF;
    RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS retention_runs_transition_guard ON retention_runs;
CREATE TRIGGER retention_runs_transition_guard
BEFORE UPDATE OR DELETE ON retention_runs
FOR EACH ROW EXECUTE FUNCTION enforce_retention_run_transition();

CREATE OR REPLACE FUNCTION reject_retention_run_item_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='retention_run_items_append_only', MESSAGE='retention run items are append-only';
END;
$$;
DROP TRIGGER IF EXISTS retention_run_items_append_only ON retention_run_items;
CREATE TRIGGER retention_run_items_append_only
BEFORE UPDATE OR DELETE ON retention_run_items
FOR EACH ROW EXECUTE FUNCTION reject_retention_run_item_mutation();
...
CREATE TABLE IF NOT EXISTS derived_artifact_deletion_audits (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    occurred_at timestamptz NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (derived_artifact_id,attempt_no,event_type),
    CHECK (event_type = 'delete_succeeded' OR NOT already_missing),
    CHECK (event_type <> 'delete_failed' OR reason_code IN ('VAULT_DELETE_FAILED','VAULT_DELETE_INTEGRITY_FAILED')),
    CHECK (event_type = 'delete_failed' OR reason_code IN ('RETENTION_EXPIRED','RIGHTS_REVOKED'))
);
ALTER TABLE derived_artifact_deletion_audits
    DROP CONSTRAINT IF EXISTS derived_artifact_deletion_audits_derived_artifact_id_fkey;
ALTER TABLE derived_artifact_deletion_audits
    ADD CONSTRAINT derived_artifact_deletion_audits_derived_artifact_id_fkey
    FOREIGN KEY (derived_artifact_id) REFERENCES derived_artifacts(id) ON DELETE RESTRICT;
ALTER TABLE derived_artifact_deletion_audits
    DROP CONSTRAINT IF EXISTS derived_artifact_deletion_audits_source_connection_id_fkey;
ALTER TABLE derived_artifact_deletion_audits
    ADD CONSTRAINT derived_artifact_deletion_audits_source_connection_id_fkey
    FOREIGN KEY (source_connection_id) REFERENCES source_connections(id) ON DELETE RESTRICT;
ALTER TABLE derived_artifact_deletion_audits
    DROP CONSTRAINT IF EXISTS derived_artifact_deletion_audits_retention_policy_id_fkey;
ALTER TABLE derived_artifact_deletion_audits
    ADD CONSTRAINT derived_artifact_deletion_audits_retention_policy_id_fkey
    FOREIGN KEY (retention_policy_id) REFERENCES source_rights_policies(id) ON DELETE RESTRICT;
CREATE INDEX IF NOT EXISTS derived_artifact_deletion_audits_artifact_idx
    ON derived_artifact_deletion_audits(derived_artifact_id,attempt_no DESC,id DESC);
...
FOR EACH ROW EXECUTE FUNCTION reject_immutable_evidence_fact_mutation();
DROP TRIGGER IF EXISTS derived_artifact_deletion_audits_append_only ON derived_artifact_deletion_audits;
CREATE TRIGGER derived_artifact_deletion_audits_append_only
BEFORE UPDATE OR DELETE ON derived_artifact_deletion_audits
FOR EACH ROW EXECUTE FUNCTION reject_immutable_evidence_fact_mutation();
...
CREATE TABLE IF NOT EXISTS evidence_lineage_reconciliation_runs (
    ...
    backup_disposition_count bigint NOT NULL DEFAULT 0 CHECK (backup_disposition_count >= 0),
...
ALTER TABLE evidence_lineage_reconciliation_runs
    ADD COLUMN IF NOT EXISTS backup_disposition_count bigint NOT NULL DEFAULT 0 CHECK (backup_disposition_count >= 0);
...
CREATE INDEX IF NOT EXISTS micro_events_lexical_fts_idx
    ON micro_events USING gin (to_tsvector('simple',((primary_subject_key || ' ') || primary_action_key)));
CREATE INDEX IF NOT EXISTS micro_events_lexical_trgm_idx
    ON micro_events USING gin (lower(((primary_subject_key || ' ') || primary_action_key)) gin_trgm_ops);
CREATE INDEX IF NOT EXISTS micro_events_location_keys_idx ON micro_events USING gin (location_keys);
CREATE INDEX IF NOT EXISTS micro_events_identifier_keys_idx ON micro_events USING gin (identifier_keys);
...
CREATE INDEX IF NOT EXISTS micro_event_summary_sentences_lexical_fts_idx
    ON micro_event_summary_sentences USING gin (to_tsvector('simple',sentence));
CREATE INDEX IF NOT EXISTS micro_event_summary_sentences_lexical_trgm_idx
    ON micro_event_summary_sentences USING gin (lower(sentence) gin_trgm_ops);
...
CREATE TABLE IF NOT EXISTS notification_outbox_events (
    ...
    event_type varchar(48) NOT NULL CHECK (event_type IN (
        ...
        'report.approval_requested','report.published'
    )),
    resource_type varchar(24) NOT NULL CHECK (resource_type IN ('micro_event','hotspot','report')),
    ...
    deep_link varchar(256) NOT NULL CHECK (deep_link ~ '^/dashboard/(events[?]event=|contents/|reports[?]report=)[1-9][0-9]{0,18}$'),
...
ALTER TABLE notification_outbox_events ADD CONSTRAINT notification_outbox_events_deep_link_check
    CHECK (deep_link ~ '^/dashboard/(events[?]event=|contents/|reports[?]report=)[1-9][0-9]{0,18}$');
...
ALTER TABLE notification_outbox_events ADD CONSTRAINT notification_outbox_events_event_type_check
    CHECK (event_type IN ('micro_event.created','micro_event.updated','micro_event.review_requested','micro_event.evidence_changed','hotspot.discovered','report.approval_requested','report.published'));
...
ALTER TABLE notification_outbox_events ADD CONSTRAINT notification_outbox_events_resource_type_check
    CHECK (resource_type IN ('micro_event','hotspot','report'));
...
CREATE UNIQUE INDEX IF NOT EXISTS user_notifications_id_user_uq ON user_notifications(id,user_id);
...
-- Read state is an append-only, per-user monotonic receipt chain. The client
-- cache is never authoritative: every cursor must name a currently visible
-- notification owned by the same active user.
CREATE TABLE IF NOT EXISTS notification_read_receipts (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    version bigint NOT NULL DEFAULT 1 CHECK (version = 1),
    user_id bigint NOT NULL REFERENCES users(id) ON DELETE RESTRICT,
    read_through_id bigint NOT NULL,
    previous_receipt_id bigint,
    recorded_at timestamptz NOT NULL DEFAULT now(),
    UNIQUE (id,user_id),
    UNIQUE (user_id,read_through_id),
    FOREIGN KEY (read_through_id,user_id) REFERENCES user_notifications(id,user_id) ON DELETE RESTRICT,
    FOREIGN KEY (previous_receipt_id,user_id) REFERENCES notification_read_receipts(id,user_id) ON DELETE RESTRICT
);
CREATE INDEX IF NOT EXISTS notification_read_receipts_user_cursor_idx
    ON notification_read_receipts(user_id,read_through_id DESC,id DESC);

CREATE OR REPLACE FUNCTION enforce_notification_read_receipt_monotonic()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
    latest_receipt_id bigint;
    latest_read_through_id bigint;
BEGIN
    PERFORM 1 FROM users WHERE id=NEW.user_id AND status='active' AND deleted_at IS NULL FOR UPDATE;
    IF NOT FOUND OR NOT EXISTS (
        SELECT 1 FROM user_notifications AS notification
        JOIN monitors AS monitor ON monitor.id=notification.monitor_id
        WHERE notification.id=NEW.read_through_id AND notification.user_id=NEW.user_id
          AND monitor.created_by=NEW.user_id AND monitor.status<>'archived' AND monitor.deleted_at IS NULL
    ) THEN
        RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='notification_read_receipts_visible_cursor',
            MESSAGE='notification read receipt requires a current visible cursor';
    END IF;
    SELECT id,read_through_id INTO latest_receipt_id,latest_read_through_id
    FROM notification_read_receipts WHERE user_id=NEW.user_id
    ORDER BY read_through_id DESC,id DESC LIMIT 1;
    IF latest_receipt_id IS NULL THEN
        IF NEW.previous_receipt_id IS NOT NULL THEN
            RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='notification_read_receipts_monotonic',
                MESSAGE='first notification read receipt cannot reference a predecessor';
        END IF;
    ELSIF NEW.previous_receipt_id IS DISTINCT FROM latest_receipt_id
       OR NEW.read_through_id <= latest_read_through_id THEN
        RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='notification_read_receipts_monotonic',
            MESSAGE='notification read receipt must advance the current cursor';
    END IF;
    RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS notification_read_receipts_monotonic ON notification_read_receipts;
CREATE TRIGGER notification_read_receipts_monotonic BEFORE INSERT ON notification_read_receipts
FOR EACH ROW EXECUTE FUNCTION enforce_notification_read_receipt_monotonic();
...
-- Immutable outcome reported by the real backup executor. Successful runs can
-- only represent a complete five-asset manifest; failed runs retain a bounded
-- reason code. Raw command output, object keys, paths and credentials are not
-- persisted in this operational fact.
CREATE TABLE IF NOT EXISTS backup_runs (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    version bigint NOT NULL DEFAULT 1 CHECK (version=1),
    run_sha256 char(64) NOT NULL UNIQUE CHECK (run_sha256 ~ '^[0-9a-f]{64}$'),
    ...
    CHECK (
        ...
        OR
        (status='failed' AND recovery_point_at IS NULL AND failure_code IN (
            'postgres_backup_failed','minio_backup_failed','vault_backup_failed',
            'manifest_incomplete','integrity_check_failed','backup_timeout','unknown'
        ))
    )
);
CREATE OR REPLACE FUNCTION reject_backup_run_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='backup_runs_append_only',
        MESSAGE='backup run facts are append-only';
END;
$$;
DROP TRIGGER IF EXISTS backup_runs_append_only ON backup_runs;
CREATE TRIGGER backup_runs_append_only BEFORE UPDATE OR DELETE ON backup_runs
FOR EACH ROW EXECUTE FUNCTION reject_backup_run_mutation();

CREATE UNIQUE INDEX IF NOT EXISTS backup_runs_id_run_sha256_uq
    ON backup_runs(id,run_sha256);

-- Append-only evidence produced only after the external backup executor has
-- destroyed an affected immutable backup copy. Paths, object keys and command
-- output remain outside PostgreSQL; the fact binds two independent operators,
-- the original backup identity and a bounded deletion-evidence digest.
CREATE TABLE IF NOT EXISTS backup_retention_dispositions (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    version bigint NOT NULL DEFAULT 1 CHECK (version=1),
    disposition_sha256 char(64) NOT NULL UNIQUE CHECK (disposition_sha256 ~ '^[0-9a-f]{64}$'),
    manifest_sha256 char(64) NOT NULL CHECK (manifest_sha256 ~ '^[0-9a-f]{64}$'),
    backup_run_id bigint NOT NULL,
    backup_run_sha256 char(64) NOT NULL UNIQUE CHECK (backup_run_sha256 ~ '^[0-9a-f]{64}$'),
    deletion_evidence_sha256 char(64) NOT NULL CHECK (deletion_evidence_sha256 ~ '^[0-9a-f]{64}$'),
    status varchar(16) NOT NULL CHECK (status='disposed'),
    reason_code varchar(32) NOT NULL CHECK (reason_code IN ('retention_expired','rights_revoked')),
    operator_record_id varchar(128) NOT NULL
        CHECK (operator_record_id=btrim(operator_record_id) AND octet_length(operator_record_id) BETWEEN 1 AND 128 AND operator_record_id !~ '[\r\n]'),
    reviewer_record_id varchar(128) NOT NULL
        CHECK (reviewer_record_id=btrim(reviewer_record_id) AND octet_length(reviewer_record_id) BETWEEN 1 AND 128 AND reviewer_record_id !~ '[\r\n]'),
    disposed_at timestamptz NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    CHECK (operator_record_id<>reviewer_record_id),
    CHECK (disposed_at <= (created_at + '00:05:00'::interval))
);
ALTER TABLE backup_retention_dispositions
    DROP CONSTRAINT IF EXISTS backup_retention_dispositions_backup_identity_fkey;
ALTER TABLE backup_retention_dispositions
    ADD CONSTRAINT backup_retention_dispositions_backup_identity_fkey
    FOREIGN KEY (backup_run_id,backup_run_sha256) REFERENCES backup_runs(id,run_sha256) ON DELETE RESTRICT;
CREATE INDEX IF NOT EXISTS backup_retention_dispositions_disposed_idx
    ON backup_retention_dispositions(disposed_at,backup_run_id);

CREATE OR REPLACE FUNCTION reject_backup_retention_disposition_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='backup_retention_dispositions_append_only',
        MESSAGE='backup retention disposition facts are append-only';
END;
$$;
DROP TRIGGER IF EXISTS backup_retention_dispositions_append_only ON backup_retention_dispositions;
CREATE TRIGGER backup_retention_dispositions_append_only
BEFORE UPDATE OR DELETE ON backup_retention_dispositions
FOR EACH ROW EXECUTE FUNCTION reject_backup_retention_disposition_mutation();

-- Immutable control evidence for an isolated projection recovery. The record
-- deliberately contains only identities, digests and counts: no notification
-- body, recipient, Vault path, secret or connection string may be persisted.
CREATE TABLE IF NOT EXISTS projection_recovery_runs (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    version bigint NOT NULL DEFAULT 1 CHECK (version=1),
    run_sha256 char(64) NOT NULL UNIQUE CHECK (run_sha256 ~ '^[0-9a-f]{64}$'),
    status varchar(16) NOT NULL CHECK (status='scheduled'),
    operator_record_id varchar(128) NOT NULL
        CHECK (btrim(operator_record_id)<>'' AND operator_record_id !~ '[\r\n]'),
    reviewer_record_id varchar(128) NOT NULL
        CHECK (btrim(reviewer_record_id)<>'' AND reviewer_record_id !~ '[\r\n]'
               AND reviewer_record_id<>operator_record_id),
    backup_evidence_sha256 char(64) NOT NULL CHECK (backup_evidence_sha256 ~ '^[0-9a-f]{64}$'),
    rehearsal_evidence_sha256 char(64) NOT NULL CHECK (rehearsal_evidence_sha256 ~ '^[0-9a-f]{64}$'),
    notification_facts_before_sha256 char(64) NOT NULL
        CHECK (notification_facts_before_sha256 ~ '^[0-9a-f]{64}$'),
    notification_facts_after_sha256 char(64) NOT NULL
        CHECK (notification_facts_after_sha256 ~ '^[0-9a-f]{64}$'
               AND notification_facts_after_sha256=notification_facts_before_sha256),
    vault_manual_before_sha256 char(64) NOT NULL
        CHECK (vault_manual_before_sha256 ~ '^[0-9a-f]{64}$'),
    vault_manual_after_sha256 char(64) NOT NULL
        CHECK (vault_manual_after_sha256 ~ '^[0-9a-f]{64}$'
               AND vault_manual_after_sha256=vault_manual_before_sha256),
    notification_outbox_count bigint NOT NULL CHECK (notification_outbox_count>=0),
    user_notification_count bigint NOT NULL CHECK (user_notification_count>=0),
    read_receipt_count bigint NOT NULL CHECK (read_receipt_count>=0),
    delivery_attempt_count bigint NOT NULL CHECK (delivery_attempt_count>=0),
    max_user_notification_id bigint NOT NULL CHECK (max_user_notification_id>=0),
    max_read_receipt_id bigint NOT NULL CHECK (max_read_receipt_id>=0),
    max_delivery_attempt_id bigint NOT NULL CHECK (max_delivery_attempt_id>=0),
    removed_delivery_claim_count bigint NOT NULL CHECK (removed_delivery_claim_count>=0),
    scheduled_vault_recovery_count bigint NOT NULL CHECK (scheduled_vault_recovery_count>=0),
    scheduled_search_rebuild_count bigint NOT NULL CHECK (scheduled_search_rebuild_count>=0),
    preserved_started_claim_count bigint NOT NULL CHECK (preserved_started_claim_count>=0),
    preserved_unknown_attempt_count bigint NOT NULL CHECK (preserved_unknown_attempt_count>=0),
    created_at timestamptz NOT NULL DEFAULT now()
);

CREATE OR REPLACE FUNCTION reject_projection_recovery_run_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE EXCEPTION USING ERRCODE='23514', CONSTRAINT='projection_recovery_runs_append_only',
        MESSAGE='projection recovery run evidence is append-only';
END;
$$;
DROP TRIGGER IF EXISTS projection_recovery_runs_append_only ON projection_recovery_runs;
CREATE TRIGGER projection_recovery_runs_append_only BEFORE UPDATE OR DELETE ON projection_recovery_runs
FOR EACH ROW EXECUTE FUNCTION reject_projection_recovery_run_mutation();
...
FOR EACH ROW EXECUTE FUNCTION reject_user_notification_fact_mutation();
DROP TRIGGER IF EXISTS notification_read_receipts_append_only ON notification_read_receipts;
CREATE TRIGGER notification_read_receipts_append_only BEFORE UPDATE OR DELETE ON notification_read_receipts
FOR EACH ROW EXECUTE FUNCTION reject_user_notification_fact_mutation();
...
-- Report lifecycle notifications use the same append-only user inbox as
-- hotspots and Product Events. Only monitor-scoped reports can name an inbox
-- owner; global reports remain available from the report workbench without
-- fabricating a recipient.
CREATE OR REPLACE FUNCTION project_report_user_notification()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    IF NEW.monitor_id IS NULL OR NEW.status NOT IN ('pending_approval','published') THEN
        RETURN NEW;
    END IF;
    SELECT monitor.id AS monitor_id,user_record.id AS user_id
    INTO recipient
    ...
    projected_event_type := CASE NEW.status
        WHEN 'pending_approval' THEN 'report.approval_requested'
        ELSE 'report.published'
    END;
    projected_title := CASE NEW.status
        WHEN 'pending_approval' THEN '日报等待审批'
        ELSE '日报已发布'
    END;
    INSERT INTO notification_outbox_events(
        event_type,resource_type,resource_id,resource_version,monitor_id,occurred_at,
        ...
        projected_event_type,'report',NEW.id,NEW.version,recipient.monitor_id,
        COALESCE(NEW.reviewed_at,NEW.submitted_at,NEW.updated_at),projected_title,
        left(NEW.title,1000),NEW.status,'/dashboard/reports?report=' || NEW.id::text,
        'report:' || NEW.id::text || ':' || NEW.status || ':' || NEW.version::text
    ) ON CONFLICT (dedupe_key) DO NOTHING
    RETURNING id INTO outbox_id;
    ...
    IF outbox_id IS NOT NULL THEN
        INSERT INTO user_notifications(
            ...
            outbox_id,recipient.user_id,recipient.monitor_id,projected_event_type,'report',
            NEW.id,NEW.version,COALESCE(NEW.reviewed_at,NEW.submitted_at,NEW.updated_at),
            projected_title,left(NEW.title,1000),NEW.status,
...
END;
$$;
DROP TRIGGER IF EXISTS reports_user_notification_projection ON reports;
CREATE TRIGGER reports_user_notification_projection
AFTER UPDATE OF status ON reports
FOR EACH ROW
WHEN (OLD.status IS DISTINCT FROM NEW.status AND NEW.status IN ('pending_approval','published'))
EXECUTE FUNCTION project_report_user_notification();
