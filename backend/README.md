...
`backend/` 是 HotKey 的后端和 OpenAPI 事实源，负责采集、标准化、相关性判断、事件智能、报告发布、订阅交付、身份权限与运行治理。

> 如果 HotKey 对你的研究、内容创作或情报工作有帮助，欢迎 Star、分享使用反馈，或从一个 Issue / Pull Request 开始参与。

## 项目组成

HotKey 由同一仓库中两个可独立开发、组合部署的子项目组成：

| 目录 | 职责 |
|------|------|
| [`backend/`](.) | 后端 API、采集与 AI 任务、数据模型、交付能力和 OpenAPI 契约 |
| [`frontend/`](../frontend/README.md) | 面向最终用户的 Web 工作台，提供事件、监控、来源、证据、报告和设置界面 |

后端可以独立运行 API 和后台任务；完整的交互体验需要同时运行 `frontend/`。
...
HOTKEY_CORS_ALLOWED_ORIGINS=http://localhost:8010,http://127.0.0.1:8010
```
...
../docs/           # Design、PRD、Plan、Acceptance、Operations 与 OpenAPI JSON
openapi/                   # 运行时 OpenAPI 注册代码
tools/                     # 构建期工具依赖
```
...
本项目基于仓库根目录的 [MIT License](../LICENSE) 开源。
