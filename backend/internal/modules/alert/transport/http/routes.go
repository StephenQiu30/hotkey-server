...
func RegisterAgentRoutes(router *gin.Engine, service alertService, authenticator httptransport.Authenticator) {
	if router == nil || service == nil {
		return
	}
	handler := NewHandler(service)
	api := router.Group("/api/v1/agent/alerts", httptransport.RequireAuthentication(authenticator), httptransport.RequireAgentScope("alerts.write"))
	api.GET("", httptransport.Wrap(handler.List))
	api.GET("/:id", httptransport.Wrap(handler.Get))
	api.POST("/:id/acknowledge", httptransport.Wrap(handler.Acknowledge))
	api.POST("/:id/resolve", httptransport.Wrap(handler.Resolve))
