...
func (service *Service) changeEnabled(ctx context.Context, input LifecycleInput, enabled bool) (*domain.ManagementSourceConnection, error) {
	...
	err := service.withTransaction(ctx, func(ctx context.Context, transaction database.Transaction) error {
		...
		if enabled && (current.SourceType == domain.SourceTypeX || current.SourceType == domain.SourceTypeBingGrounding || current.SourceType == domain.SourceTypeBilibili || current.SourceType == domain.SourceTypeWeibo || current.SourceType == domain.SourceTypeGoogleAgentSearch) && current.HealthStatus != domain.HealthStatusHealthy {
			return domain.SourceConnectionUnavailable()
		}
...
func normalizeCreate(connection domain.SourceConnection) (domain.SourceConnection, error) {
	if connection.SourceType != domain.SourceTypeRSS && connection.SourceType != domain.SourceTypeHackerNews && connection.SourceType != domain.SourceTypeX && connection.SourceType != domain.SourceTypeBingGrounding && connection.SourceType != domain.SourceTypeBilibili && connection.SourceType != domain.SourceTypeWeibo && connection.SourceType != domain.SourceTypeGoogleAgentSearch {
	...
	if connection.SourceType == domain.SourceTypeX || connection.SourceType == domain.SourceTypeBingGrounding || connection.SourceType == domain.SourceTypeBilibili || connection.SourceType == domain.SourceTypeWeibo || connection.SourceType == domain.SourceTypeGoogleAgentSearch {
